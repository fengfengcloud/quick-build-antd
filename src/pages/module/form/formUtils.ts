 ...
 */
export const getDifferentField = ({ dest, sour, moduleInfo }:
    { dest: object, sour: object, moduleInfo: ModuleModal }) => {
    console.log('当前的所有字段');
    console.log(sour);
    console.log('修改后所有字段');
    console.log(dest);
    ...
    for (let key in result) {
        if (result[key] === '')            //为空的字符串全保存为null
            result[key] = null;
        const field: ModuleFieldType = getFieldDefine(key, moduleInfo);
        if (field && field.isDateField) {
            if (!result[key]) {            //原来是空，现在也是空
                if (!sour[key]) {
                    delete result[key];
                }
            } else
                if (result[key].isSame(sour[key], DateTimeFormat))
                    delete result[key];
        } else {
            if (result[key] === sour[key]) {
                delete result[key];
            }
        }
    }
 ...
 * @param moduleState
 */
export const getNewDefaultValues = (form: FormInstance, moduleState: ModuleState, setV?: Function): object => {
    const result = {};
    const { moduleName, dataSource,
        filters: { navigate, parentfilter: pf },
        selectedRowKeys,
        lastInsertRecord } = moduleState;
        ...
        if (!ahead || ahead.indexOf('.') == -1) {
            ...
            if (key in fields) {
                ...
                if (ahead) {
                    ...
                    if (namefieldname in fields)
                        result[namefieldname] = rec.text;
                }
            }
        }
    })
    ...
    // 去服务器后台取得后台提供的缺省值
    if (navigate.length)
        params.navigates = JSON.stringify(navigate);
    if (pf)
        params.parentfilter = JSON.stringify(pf);
