...
import {
  InputNumber, DatePicker, Popover, Tabs, Space,
  Input, Form, Checkbox, Select, Radio, Row, Col, Card, TreeSelect, Rate, AutoComplete
} from 'antd';
import { Dispatch } from 'redux';
import {
  BlockOutlined, DownOutlined, RightOutlined, ProfileOutlined,
  AuditOutlined, FileDoneOutlined, SearchOutlined
} from '@ant-design/icons';
import { ModuleModal, ModuleFieldType, FormOperateType, FormShowType } from '../data';
import { getDictionary, getPropertys } from '../dictionary/dictionarys';
import { TextValue } from '../data';
import {
  getModuleInfo, getModuleComboDataSource, getModulTreeDataSource, getFieldDefine,
  addParentAdditionField, getModulTreePathDataSource
} from '../modules';
import styles from './formFactory.less';
import { FormInstance, Rule } from 'antd/lib/form';
import { DateFormat, DateTimeFormat, DateTimeFormatWithOutSecond, getModuleNameFromOneToMany } from '../moduleUtils';
...
export const getFormSchemePanel: React.FC<FormSchemePanelProps> = (params): any => {
  const { details, moduleInfo, form, fieldsValidate, readOnly, dispatch, showType,
    parentCols, setV, formType, currRecord } = params;
  const items: any[] = [];
  ...
  details.forEach((item: any) => {
    ...
    if (subobjectid) {
      ...
      // 只要可审批，不管在什么状态下都可以进行
      if (canApprove(currRecord)) {                 //formType == 'approve' && 
    ...
    } else if (fieldid) {                          // 有字段id的，说明是一个字段，不是
      field = <FormField
        formFieldDefine={item} moduleInfo={moduleInfo} dispatch={dispatch}
        fieldsValidate={fieldsValidate} form={form} readOnly={readOnly}
        formType={formType} currRecord={currRecord} />;
      ...
      const tabs: any = [];
      for (let index in item.details) {
        const detail = item.details[index];
        tabs.push(<TabPane key={detail.detailid}
          tab={detail.iconCls ?
      ...
      cardParams.className = showType + '-' + xtype + (cardParams.className ? ' ' + cardParams.className : '');
      if (!item.tabTitle && title) {      //如果有tabTitle,说明是tab的组件，不需要这里有标题
        // 有 title 才有当前panel的标题，才可以折叠
        cardParams.title = ((item.hiddenTitle === true && showType !== 'mainregion') ? null :
          <Space>{cardParams.icon}<span>{title}</span></Space>);
        field = cols ?
          <Card {...cardParams}>
            <span style={collapsed ? { display: 'none' } : {}}>
        ...
        field = cols ?
          <Card {...cardParams}><Row gutter={16}>{children}</Row></Card > :
          <Card {...cardParams}>{children}</Card >;
      }
      firstFieldSet = false;
    } else if (xtype == 'container') {
      field = <span style={{ padding: 12 }} dangerouslySetInnerHTML={{ __html: item.html }}></span>;
    }
...
const getPropertyValueInput: React.FC<FormFieldProps> = ({ fieldDefine, fieldProps }) => {
  ...
  return <AutoComplete  {...fieldProps} allowClear showSearch
    getPopupContainer={triggerNode => triggerNode.parentNode}
    filterOption={(input, option: any) => {
      return option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
    }}>
    {fieldDefine.propertyvalue?.split(',').map((text, index) =>
      <Select.Option key={'key-' + index} value={text || ''}>{text}</Select.Option>)
    }
  </AutoComplete>
}
 ...
 * @param param0 
 */
const getDictionaryInput: React.FC<FormFieldProps> = ({ fieldDefine, formFieldDefine, fieldProps }) => {
  if (fieldProps.readOnly) {
    return <Input {...fieldProps} />
  }
  ...
  if (inputmethod == '40')
    return <Radio.Group {...fieldProps}>
      {data.map(({ value, text }) => formFieldDefine.radioButton ?
        <Radio.Button key={'key-' + value} value={value}>{text}</Radio.Button> :
        <Radio key={'key-' + value} value={value}>{text}</Radio>)}
    </Radio.Group>
  else
    return <Select  {...fieldProps} allowClear
      showSearch={inputmethod != '10'}
      getPopupContainer={triggerNode => triggerNode.parentNode}
      filterOption={(input, option: any) => {
        return option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
      }}>
      {data.map(({ value, text }) => <Select.Option key={'key-' + value} value={value || ''}>
        {(inputmethod == '30' ? value + '-' : '') + text}</Select.Option>)}
    </Select>
}
...
const ManyToOneSelectPopover = ({ fieldDefine, form }:
  ...
  return <Popover
    ...
    content={<SelectGrid manyToOneInfo={{
      form: form,
      ...
      moduleName={fieldDefine.fieldtype}></SelectGrid>}>
    <SearchOutlined />
  </Popover>
}
...
const getManyToOneInput: React.FC<FormFieldProps> = params => {
  ...
  if (fieldProps.readOnly) {
    return <Input {...fieldProps}
      suffix={
        <PopoverDescriptionWithId id={currRecord[getFieldName(fieldDefine)]}
          moduleInfo={cobject} dispatch={dispatch} />
      } />
  }
  ...
  if (mode == '95') {
    ...
    if (radioItems.length <= 30)
      return <Radio.Group {...fieldProps}>{radioItems}</Radio.Group>
    mode = '20';
  }
  if (mode == '10' || mode == '20' || mode == '30') {
    ...
    // 20	可录入关键字选择(local)
    // 30	可录入编码和关键字选择(local)
    const options = getModuleComboDataSource(fieldtype)
      .map(({ value, text }) => ({
        value,
        label: (mode == '30' ? value + '-' : '') + text,
      }));
    return <Select allowClear
      showSearch={mode !== '10'} {...fieldProps} options={options}
      getPopupContainer={triggerNode => triggerNode.parentNode}
      filterOption={(input, option: any) => {
  ...
  if (mode == '60') {
    const arrageTreeNode = (array: any): TextValue[] => {
      return array.map((rec: TextValue) => ({
        ...
        children: rec.children && rec.children.length > 0 ? arrageTreeNode(rec.children) : undefined
      }));
    };
    const manytooneData = arrageTreeNode(
      getModulTreeDataSource(fieldtype, !!allowParentValue, !!formFieldDefine.addCodeToText));
    return <TreeSelect {...fieldProps} treeDefaultExpandAll
      allowClear showSearch treeNodeFilterProp="label" treeData={manytooneData}
      getPopupContainer={triggerNode => triggerNode.parentNode} />
  }
  ...
  if (mode == '70') {
    ...
    const manytooneData = arrageTreeNode(
      getModulTreePathDataSource(fieldtype));
    return <TreeSelect {...fieldProps} treeDefaultExpandAll
      allowClear showSearch treeNodeFilterProp="label" treeData={manytooneData}
      getPopupContainer={triggerNode => triggerNode.parentNode} />
  }
...
const getManyToManyInput: React.FC<FormFieldProps> = params => {
  ...
  return formFieldDefine.xtype == 'manytomanycheckboxgroup' ?
    <Checkbox.Group {...fieldProps}>
      {getModuleComboDataSource(getModuleNameFromOneToMany(fieldDefine.fieldtype)).map((rec: any) =>
        <Checkbox key={rec.value} value={rec.value} >{rec.text}</Checkbox>)}
    </Checkbox.Group> :
    <Select mode="multiple" allowClear {...fieldProps}
      getPopupContainer={triggerNode => triggerNode.parentNode}
      filterOption={(input, option: any) => {
        return option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
      }}>
      {getModuleComboDataSource(getModuleNameFromOneToMany(fieldDefine.fieldtype)).map((rec: any) =>
        <Select.Option key={rec.value} value={rec.value} label={rec.label}>{rec.text}</Select.Option>)}
    </Select>
}

export const getPercentFormField = (digitslen: number, fieldProps: any) => {
  return !fieldProps.readOnly ? <InputNumber
    precision={digitslen}
    step={0.01}
    formatter={(value: any) => {
      if (!value) return '';
      ...
      if (Number.isFinite(value))
        v = (value * 100).toFixed(digitslen - 2);
      else {
    ...
    parser={(value: string) => {
      let v = value.replace('%', '')
      let floatvalue = (Number.parseFloat(v) / 100).toFixed(digitslen);
      return floatvalue + '|||' + v
    }}
    style={{ width: '100px' }}
    {...fieldProps}
  ...
  switch (fieldtype.toLowerCase()) {
    case 'integer':
      if (fieldDefine.isRate) {
        ...
        formField = <InputNumber precision={0} className='integer'
          formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
          parser={(value: any) => value.replace(/\$\s?|(,*)/g, '')}
          style={{ width: '120px' }} {...fieldProps} />
      }
    ...
    case 'percent':
      ...
      break;
    case 'date':
      formField = <DatePicker format={DateFormat} {...fieldProps}
        placeholder={fieldProps.readOnly ? "" : undefined} />
      break;
    case 'datetime':
    case 'timestamp':
      const dateFormat = fieldDefine.disableSecond === false ? DateTimeFormat : DateTimeFormatWithOutSecond;
      formField = <DatePicker showTime format={dateFormat} {...fieldProps}
        placeholder={fieldProps.readOnly ? "" : undefined} />
      break;
    case 'boolean':
      formField = <Checkbox {...fieldProps} ></Checkbox>
      break;
    case 'image':
      formField = <ImageField {...fieldProps}
        imageWidth={formFieldDefine.imageWidth}
        imageHeight={formFieldDefine.imageHeight}
        imageStyle={formFieldDefine.imageStyle} />
      break;
    case 'string':
      const len = fieldname == 'creater' || fieldname == 'lastmodifier' ? 10 : fieldDefine.fieldlen;
      if (len > 0 && len <= 100)      // allowClear={!fieldDefine.isrequired}
        formField = <Input maxLength={len} {...fieldProps}
          style={len <= 10 ? { maxWidth: (len * 16 + 24) + 'px' } : {}} />
      else if (len == 0)
        formField = <Input.TextArea autoSize {...fieldProps} />
      else
        formField = <Input.TextArea maxLength={len}
          autoSize={{ maxRows: 10 }} {...fieldProps} />
      break;
    default:
...
const getFieldName = (field: ModuleFieldType, readOnly: boolean | undefined = undefined) => {
  ...
  else if (field.fDictionaryid && readOnly)
    return field.fieldname + '_dictname';
  else
    return field.fieldname;
}

const FormField = ({ formFieldDefine, moduleInfo, fieldsValidate, form,
  readOnly, formType, currRecord, dispatch }:
  {
    formFieldDefine: any, moduleInfo: ModuleModal, fieldsValidate: any, dispatch: Dispatch,
    form: any, readOnly: boolean, formType: FormOperateType, currRecord: any
  }): any => {
  let fieldDefine: ModuleFieldType = getFieldDefine(formFieldDefine.fieldid, moduleInfo);
  if (fieldDefine == null) {
    if (formFieldDefine.fieldahead) { // 如果是父模的其他字段或者祖父模块
      if (formFieldDefine.aggregate) {        //子模块的暂未考虑    
        //return null;
      } else {
  ...
  const { fieldname, fieldtype,
    ...
    isrequired, allownew, allowedit,
  } = fieldDefine;
  let fieldtitle = getLastLevelLabel(fieldDefine.fieldtitle);
  ...
  if (formFieldDefine.fieldahead || formType == 'display' || formType == 'approve' || formType == 'audit'
    || formType == 'insert' && !allownew || formType == 'edit' && !allowedit) {
    readOnly = true;
  ...
  const formItemProp: any = {
    ...
    name: getFieldName(fieldDefine, readOnly),
  ...
  const fieldProps: any = {
    ...
    readOnly,                                 // 字段禁用时不显示placeholder
    disabled: readOnly && (formType == 'insert' || formType == 'edit'),
    placeholder: readOnly ? undefined : formFieldDefine.placeholder || fieldDefine.placeholder,
    fielddefaultvalue: readOnly ? undefined : defaultvalue,       // 新建时的缺省值,如果有问题，则改为formFieldDefine.fieldahead
    addonAfter: unittext,
  }
  const rules: Rule[] = [];
  if (!readOnly) {
    if (required)
      rules.push({
        required,
        message: formFieldDefine.requiredMessage || fieldDefine['requiredMessage'] || '请输入' + fieldtitle
      })
  ...
  if (fieldDefine.isManyToOne)
    pobject = getModuleInfo(fieldtype);
  if (fieldDefine.isManyToOne && !fieldDefine.fieldahead && !readOnly && pobject?.selectedmode == '90') {
    formItemProp.name = fieldDefine.manyToOneInfo.nameField;
    fieldItem = <>
      <FormItem label={fieldtitle} {...formItemProp} rules={rules} >
        <Input {...fieldProps} readOnly
          addonAfter={<ManyToOneSelectPopover fieldDefine={fieldDefine} form={form} />} >
        </Input>
      </FormItem>
    ...
    const fieldinput = getFieldInput({
      moduleInfo, fieldDefine, formFieldDefine, form,
      fieldProps, currRecord, dispatch
    })
    if (unittext && fieldtype.toLowerCase() !== 'string')
      fieldItem = <FormItem label={fieldtitle}
        {...formItemProp} key={formFieldDefine.detailid + '1'}>
      ...
      fieldItem = <FormItem label={fieldtitle} {...formItemProp} rules={rules} >
        {fieldinput}
      </FormItem>
  }
  // 当前字段根据另外的字段的值来显示或隐藏
  ...
  if (formFieldDefine.parentField) {
    ...
    return <Form.Item noStyle
      shouldUpdate={(prevValues, currentValues) => prevValues[fieldName] !== currentValues[fieldName]}
    >
      {(params) => {
