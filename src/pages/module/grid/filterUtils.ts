...
import {
    getUserFilterCount, stringFieldOperator,
...
export const getSqlparamFilter = (stateSqlparam: any) => {
    if (!stateSqlparam)
        return {};
    const sqlparam = {};
    for (let key in stateSqlparam) {
        sqlparam[key] = stateSqlparam[key].value;
    }
    for (let key in sqlparam) {
        if (isMoment(sqlparam[key])) {
            sqlparam[key] = sqlparam[key].format("YYYY-MM-DD");
        }
    }
    return sqlparam;
}
 ...
 */
export const getAllFilterAjaxParam = (filters: ModuleFilters) => {
    const payload: any = {};
...
export const getAllFilterAjaxText = (moduleState: ModuleState): any[] => {
    ...
    result.push(...changeUserFilterToParam(filters.userfilter, true, ',').
        map((f: any) => {
            const result = { ...f };
    ...
    result.push(...getGridColumnFiltersDescription(filters.columnfilter || [],
        getColumnFiltersInfo(moduleName), ','))

    result.push(...filters.navigate.map((filter: any) => ({
    ...
    for (let i in columnFilter) {
        if (columnFilter[i].property === columnKey)
            if (columnFilter[i].value)
                return columnFilter[i].value.split('--');
            else
                return [null, null];
    }
    return null;
...
const DateFilterSelectOption = [
    { value: 'daysection', text: '区间' }
]
