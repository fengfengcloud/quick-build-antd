...
const GridSizeButton = ({ moduleState, dispatch }: { moduleState: ModuleState, dispatch: Dispatch }) => {
    const { moduleName, currSetting: { gridSize } } = moduleState;
    const getIcon = (key: string, title: string) =>
        <Menu.Item key={key}>
            <span style={{ paddingRight: '48px' }}>
                {gridSize == key ? <CheckOutlined /> : hiddenIcon}{title}
            </span>
        </Menu.Item>
    const menu = (
        <Menu onClick={({ key }) => {
