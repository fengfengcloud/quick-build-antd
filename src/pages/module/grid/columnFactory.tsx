...
import { Tooltip, Button, Input, Popover, Select, Divider, Radio, DatePicker } from 'antd';
import {
  PaperClipOutlined, BarsOutlined, SearchOutlined, InfoCircleOutlined, AuditOutlined, CheckCircleOutlined,
} from '@ant-design/icons';
import { ModuleModal, ModuleFieldType, ColumnFilterInfoType, ModuleState, TextValue, GridOperateType, } from '../data';
import styles from './columnFactory.less';
import { getSortOrder } from './sortUtils';
import {
  nameFieldRender, booleanRenderer, dateRender, datetimeRender, directionaryFieldRender,
  manyToOneFieldRender, imageRender, oneToManyFieldRender, integerRender, floatRender,
  percentRender, monetaryRender, manyToManyFieldRender, treeNodePinFieldRender,
  rateRender, iconClsRender, stringRenderer, longstringRenderer, percentRenderWithTooltip
} from './columnRender';
import {
  getColumnFilterValue, getBooleanFilterOption, getStringColumnFilterValue,
  getNumberColumnFilterValue, getColumnFilterInfo, NumberFilterSelectOption, getDateColumnFilterValue
} from './filterUtils';
import { apply, applyIf, applyOtherSetting } from '@/utils/utils';
import { Dispatch } from 'redux';
import { getDictionaryData } from '../dictionary/dictionarys';
import { getModuleInfo, getModuleComboDataSource, addParentAdditionField, getFieldDefine } from '../modules';
...
export const getSubTotalFields = (items: any[], namefield: string) => {
  const result: any[] = [];
  const genFields = (items: any[]) => {
    items.forEach((item: any) => {
      if (item.children) {
        genFields(item.children);
      } else {
        if (item.dataIndex && item.fieldDefine && item.fieldDefine.allowsummary)
          result.push(item);
...
export const getColumns = ({ moduleInfo, moduleState, gridScheme, dispatch, gridType, readOnly }:
  {
    moduleInfo: ModuleModal, moduleState: ModuleState, gridScheme: any, dispatch: Dispatch, gridType: GridOperateType,
    readOnly?: boolean
  }): any => {
  console.log("getcolumns");
  let columns: any[] = getLockedLeftColumns(moduleInfo, moduleState, dispatch);
  let result = columns.concat(getGroupAndFieldColumns({ moduleState, moduleInfo, columns: gridScheme.columns, dispatch, gridType }));
  moduleExportGridColumnDefine[moduleInfo.modulename] = getExportGridColumns(result);

  if (!readOnly)  // && moduleName !== 'VActRuTask' && moduleName !== 'VActFinishTask'
    result.push(getActionColumn({ moduleInfo, moduleState, dispatch }));

  return result;
}

const getGroupAndFieldColumns = ({ moduleInfo, moduleState, columns, dispatch, gridType }:
  { moduleInfo: ModuleModal, moduleState: ModuleState, columns: [], dispatch: Dispatch<any>, gridType: GridOperateType }) => {
  ...
  for (let i in columns) {
    ...
    if ((column.columns && column.columns.length) || !column.fieldid) { // 没有fieldid就当作合并表头
      ...
      let group: any = {
        ...
        children: getGroupAndFieldColumns({ moduleState, moduleInfo, columns: column.columns, dispatch, gridType })
      }
      ...
      if (column.fieldahead) { // 附加字段
        if (column.aggregate) {
          ...
          if (field) {
            const gc: any = getColumn({ moduleState, gridField: column, fieldDefine: field, moduleInfo, dispatch, gridType });
          ...
          if (field) {
            const gc = getColumn({ gridField: column, fieldDefine: field, moduleInfo, moduleState, dispatch, gridType });
            if (gc) result.push(gc);
          }
        }
      } else {
        ...
        let gc = getColumn({ gridField: column, fieldDefine: field, moduleInfo, moduleState, dispatch, gridType });
        if (gc) result.push(gc);
      }
    }
  }
 ...
 */
const getColumn = ({ gridField, fieldDefine, moduleInfo, moduleState, dispatch, gridType }:
  {
    gridField: any, fieldDefine: ModuleFieldType, moduleInfo: ModuleModal,
    moduleState: ModuleState, dispatch: Dispatch, gridType: GridOperateType
  }) => {
  ...
  if (moduleInfo.namefield == fieldDefine.fieldname) {
    field.render = (value: any, record: object, recno: number) =>
      nameFieldRender(value, record, recno,
        { moduleInfo, dispatch });
  }
  ...
  if (fieldDefine.digitslen) {
    ...
    let ds = '';
    for (let i = 0; i < fieldDefine.digitslen; i++)
      ds += '0';
  ...
  if (fieldDefine.isManyToOne || fieldDefine.isOneToOne) {
    ...
    field.render = (value: any, record: object, recno: number) =>
      manyToOneFieldRender(value, record, recno, {
        ...
        keyField: fieldDefine.manyToOneInfo.keyField, dispatch
      });
  ...
  if (fieldDefine.isOneToMany) {
    ...
    field.render = (value: any, record: Object, recno_: number) =>
      oneToManyFieldRender(value, record, recno_,
        {
          fieldtitle: field.menuText, childModuleName: field.childModuleName,
          fieldahead: field.fieldahead, moduleInfo, dispatch
        });
  }
  if (fieldDefine.aggregate) {
    ...
    field.isOneToMany = true;
    field.childModuleName = childparts[1];
    ...
    field.render = (value: any, record: Object, recno_: number) =>
      oneToManyFieldRender(value, record, recno_,
        {
          fieldtitle: field.menuText, childModuleName: field.childModuleName, dispatch,
          fieldahead: field.fieldahead, moduleInfo
        });
  }
  ...
  field.title = <span style={{ whiteSpace: 'nowrap' }}
    dangerouslySetInnerHTML={{ __html: field.title }}>
  </span>;
  // 设置了title是一个icon
  if (gridField.titleIconCls) {
    field.title = <Tooltip title={field.menuText}>
      <span className={gridField.titleIconCls}></span>
    </Tooltip>
    field.align = 'center';
  }
  ...
  if (gridField.columnRender) {
    ...
    field.render = (value: any, record: Object, recno_: number) =>
      <span dangerouslySetInnerHTML={{ __html: field.columnRender(value, record, recno_) }}></span>
  }
  ...
  if (field.disableSorter)
    field.sorter = false;
  if (field.disableSearch || gridType == "onetomanygrid") {
    delete field.filterDropdown;
    delete field.filters;
  }
  if (gridType == "onetomanygrid" && fieldDefine.fieldname == moduleState.filters.parentfilter?.fieldahead)
    return null;
  ...
  if (businessRender) {
    field.render = (value: any, record: any, recno: number) =>
      businessRender({ value, record, recno, dataIndex: field.dataIndex, fieldDefine, dispatch, moduleState })
  }
  return field;
}
...
export const getLockedLeftColumns = (moduleInfo: ModuleModal, moduleState: ModuleState, dispatch: Dispatch<any>) => {
  ...
  if (moduleInfo.moduleLimit.hasapprove) {
    // 四合一的审批按钮
    columns.push({
      title: <Tooltip title="审批流程"><AuditOutlined /></Tooltip>,
  ...
  if (moduleInfo.moduleLimit.hasaudit) {
    columns.push({
      title: <Tooltip title="审核信息"><CheckCircleOutlined /></Tooltip>,
...
const setFieldxtype = (field: any, fieldtype: string, moduleState: ModuleState,
  ...
  if (field.fieldDefine.aggregate) {
    ...
    case 'date':
      ...
      apply(field, getDateColumnSearchProps(field.dataIndex, field.fieldDefine));
      break;
    case 'datetime':
    case 'timestamp':
      apply(field, {
    ...
    case 'percent':
      ...
      if (field.fieldDefine.divisor) {
        field.render = (value: number, record: any, _recno: number) =>
          percentRenderWithTooltip(value, record && record[field.fieldDefine.divisor],
    ...
    case 'blob':
      ...
      break;
    case 'string':
      const len = field.fieldDefine.fieldlen;
      if (len > 0 && len <= 100)
        field.render = stringRenderer;
      else
        field.render = longstringRenderer;
      break;
    default:
  }
}
...
const getDateColumnSearchProps = (dataIndex: any, fieldDefine: any) => ({
  filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }:
    { setSelectedKeys: any, selectedKeys: any, confirm: any, clearFilters: any }) => (
    <div style={{ padding: 8, width: '300px' }}>
      <div style={{ marginBottom: 8, display: 'block' }}>
        <DateSectionQuickSelect callback={(value: any) =>
          setSelectedKeys([value[0].format(DateFormat), value[1].format(DateFormat)])} />
        <RangePicker allowEmpty={[true, true]} picker='date' style={{ flex: 1 }} format={DateFormat}
          value={[selectedKeys[0] ? moment(selectedKeys[0], DateFormat) : null,
          ...
          <span>
            <Input.Group compact>
              <Select
                ...
                style={{ width: 90 }}>
                {NumberFilterSelectOption.map((item) => <Option key={item.value} value={item.value}>{item.text}</Option>)}
              </Select>
              <Input
                ref={node => {
                  searchInput = node;
              ...
              <Popover trigger="click" title="数值字段筛选说明"
                ...
                }><a><InfoCircleOutlined style={{ margin: 8 }} /></a></Popover>
            </Input.Group>
          </span>
          <div style={{
            display: 'flex',
