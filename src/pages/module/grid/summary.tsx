...
export const tableSummary = (pageData: any[], moduleState: ModuleState, subTotalFields: any[]) => {
    ...
    const getFieldTotal = (field: any) => {
        ...
        if (fieldDefine.ismonetary)
            return monetaryRender(sum, {}, 0, moduleState);
        else if (fieldDefine.divisor && fieldDefine.denominator) {
            const divisor = getTotal(fieldDefine.divisor);
            const denominator = getTotal(fieldDefine.denominator);
            return percentRenderWithTooltip(denominator != 0 ? divisor / denominator : 0, divisor, denominator)
        } else
    ...
    const getRemoteFieldTotal = (field: any) => {
        ...
        if (fieldDefine.ismonetary)
            return monetaryRender(value, {}, 0, moduleState);
        else if (fieldDefine.divisor && fieldDefine.denominator) {
    ...
    return (pageData.length == 0 ? null : (
        <><Table.Summary.Row key={moduleState.moduleName + '_summary'}>
            {subTotalFields.map((field: any, index: number) => {
                ...
                if (field) {
                    if (field.namefield) {
                        ...
                        value = <span className={styles.totalfield}>
                            <Text strong>
                        ...
                        value = <Text strong style={{
                            float: field.fieldDefine.divisor ? 'left' : 'right',    //百分比靠左对齐，数值靠右
                        }}>
                            {getFieldTotal(field)}
                        </Text>
                }
                return <Table.Summary.Cell className={styles.summarycell}
                    key={moduleState.moduleName + '_summary' + index}
            ...
            {
                moduleState.remoteRoot && Object.getOwnPropertyNames(moduleState.remoteRoot).length &&
                    ...
                    <Table.Summary.Row key={moduleState.moduleName + '_total'}>
                        {subTotalFields.map((field: any, index: number) => {
                            ...
                            return <Table.Summary.Cell className={styles.totalcell}
                                key={moduleState.moduleName + '_total' + index}
                                index={index} colSpan={colSpan}>
                                {value}
                            </Table.Summary.Cell>
                        })
