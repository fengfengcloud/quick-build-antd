...
import { Button, Card, Form, InputNumber, message, Modal, Switch, Tabs, Tooltip, Typography } from 'antd';
...
export const UpdateRecordOrderNoButton: React.FC<params> = ({ moduleState, dispatch }) => {
        ...
        request('/api/platform/dataobject/updateorderno.do',
            {
                ...
                if (result.success) {
                    ...
                    dataSource.forEach(record => {
                        record[result.tag] = result.msg[i++].text;
                    });
                    dispatch({
        ...
        request('/api/platform/dataobject/updatepageorderno.do',
            {
                ...
                if (result.success) {
                    let i = 0;
                    dataSource.forEach(record => {
                        record[result.tag] = result.msg[i++];
                    });
                    dispatch({
                        type: 'modules/updateDataSource',
                        payload: {
    ...
    return <React.Fragment>
        ...
        <Modal visible={visible}
            title={<><QuestionCircleOutlined /> 确认更新顺序号</>}
            ...
            <Tabs activeKey={activekey} onTabClick={(key: any) => setActiveKey(key)} >
                <Tabs.TabPane tab='重新生成顺序号' key='reset'>
                    <Typography>
                        ...
                        <Typography.Paragraph>
                            注意：只会对当前页的记录进行操作更新，其他页的记录将不会进行更新；<br />
                    ...
                    <Card bodyStyle={{ padding: '12px' }}>
                        <Form form={form} className='moduleform'
                            labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}
                        >
                            <Form.Item label='加入父记录顺序值' name='addparent' style={{ marginBottom: '8px' }}>
                                <Switch />
                            </Form.Item>
                            <Form.Item label='起始顺序号' name='startnumber' initialValue={10} style={{ marginBottom: '8px' }}>
                                <InputNumber />
                            </Form.Item>
                            <Form.Item label='递进值' name='stepnumber' initialValue={10} style={{ marginBottom: '8px' }}>
