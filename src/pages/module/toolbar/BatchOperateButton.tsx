...
const BatchOperateButton = ({ moduleState, dispatch, readOnly }:
    {
        moduleState: ModuleState, dispatch: Dispatch, readOnly?: boolean,
    }) => {
     ...
     */
    const checkAllCanDelete = (moduleState: ModuleState) => {
        const { moduleName } = moduleState;
        const moduleInfo = getModuleInfo(moduleName);
        const items: string[] = [];
        getSelectedRecords(moduleState).forEach((record: any) => {
            const r = canDelete(moduleInfo, record);
            if (!r.canDelete)
        ...
        if (items.length) {
            notification.warning({
                ...
                description: items.map((text, index) => <div>{`${index + 1}、${text}`}</div>),
                style: { width: '500px' },
            })
        }
        return !items.length;
    ...
    const menu = <Menu>
        {hasDelete(moduleInfo) && !readOnly ?
            ...
            >删除选中的 {count} 条记录</Menu.Item> : null}
        {hasDelete(moduleInfo) && !readOnly ? <Menu.Divider /> : null}
        {!readOnly ? getAdditionFunctionItems() : null}
        <Menu.Item key="toExcel" onClick={() => {
            downloadGridSchemeFile({ moduleState, key: 'toExcel', topdf: false, onlyselected: true })
        }}><FileExcelOutlined />选中记录导出Excel文档
        <Tooltip title="导出Pdf文件" placement="topRight">
                <Button style={{ float: 'right', paddingRight: '5px' }} type="link" size="small"
                    onClick={(e) => {
                        e.stopPropagation();
                        setVisible(false);
                        downloadGridSchemeFile({ moduleState, key: 'toExcel', topdf: true, onlyselected: true })
                    }}
        ...
        <Menu.Item key='none' icon={spaceIcon} onClick={() => {
            dispatch({
 ...
 * @param param0 
 */
export const getToolbarButton = ({ moduleState, dispatch }: {
    ...
    const funs = functions.filter((fun: AdditionFunctionModal) => {
        ...
        if (min > 0 && moduleState.selectedRowKeys.length === 0)
            return false;
        return fun.menuname === 'toolbar' &&
