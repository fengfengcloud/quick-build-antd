...
import { ModuleModal, ModuleState, ModuleFilters, TextValue } from '../../data';
...
export const downloadGridSchemeFile = ({ moduleState, key, topdf, onlyselected }:
    { moduleState: ModuleState, key: string, topdf: boolean, onlyselected: boolean }) => {
    ...
    if (onlyselected) {
        ...
        apply(params, getAllFilterAjaxParam(moduleState.filters));
...
const ExportGridScheme = ({ moduleState, setVisible, onlyselected }:
    { moduleState: ModuleState, setVisible: Function, onlyselected: boolean }) => {
    ...
    const getGridExcelExportItems = (moduleInfo: ModuleModal) => {
        ...
        const result: any[] = [];
        const items: any[] = formschemes.filter((scheme) => scheme.formtype === GRIDEXCELEXPORT).map((scheme) => {
            ...
            return <Menu.Item key={key} onClick={() => {
                ...
                <Tooltip title="导出pdf文件" placement="topRight">
                    <Button style={{ float: 'right', paddingRight: '0px' }} type="link" size="small"
                        onClick={(e) => {
                            e.stopPropagation();
                            setVisible(false);
        ...
        if (items.length > 0) {
            result.push(<Menu.ItemGroup title="列表导出方案" key="_export_grid_key_">
                {items}
            </Menu.ItemGroup>)
    ...
    return getGridExcelExportItems(moduleInfo)
