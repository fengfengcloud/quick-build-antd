...
const ExportButton = ({ moduleState, dispatch }:
    { moduleState: ModuleState, dispatch: Dispatch }) => {
    const [visible, setVisible] = useState(false);
    const menu = <Menu key="exportButton">
        <Menu.Item key="toExcel" onClick={() => {
            downloadGridSchemeFile({ moduleState, key: 'toExcel', topdf: false, onlyselected: false })
        }}><FileExcelOutlined />列表导出Excel文档
        <Tooltip title="导出pdf文件" placement="topRight">
                <Button style={{ float: 'right', paddingRight: '5px' }} type="link" size="small"
                    onClick={(e) => {
                        e.stopPropagation();
                        setVisible(false);
                        downloadGridSchemeFile({ moduleState, key: 'toExcel', topdf: true, onlyselected: false })
                    }}
    ...
    return <Dropdown overlay={menu} visible={visible} onVisibleChange={(v: boolean) => setVisible(v)}>
        <Button>
            导出  <DownloadOutlined />
        </Button>
    </Dropdown>
