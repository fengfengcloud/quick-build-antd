...
const ExportRecordScheme = ({ moduleState, setVisible }:
    ...
    const getRecordExcelExportItems = (moduleInfo: ModuleModal) => {
        ...
        const items: any[] = excelSchemes.map((scheme) =>
            <Menu.Item key={RECOREXPORT + "||" + scheme.schemeid + "||" + scheme.title}
                ...
                <Tooltip title="下载pdf文件" placement="topRight">
                    <Button style={{ float: 'right', marginRight: '0px', paddingRight: '0px' }}
                ...
                <Tooltip title="新窗口中打开pdf文件" placement="topRight">
                    <Button style={{ float: 'right', margin: '0px', padding: '0px' }}
                        type="link" size="small"
                        onClick={(e: any) => {
                            e.stopPropagation();
                            setVisible(false);
        ...
        if (items.length > 0) {
            result.push(<Menu.ItemGroup title="记录导出方案" key="_export_record_key_">
                {items}
            </Menu.ItemGroup>)
     ...
     * @param key 
     */
    const recordExport = ({ schemeid, title, topdf, openinwindow }:
        { schemeid: string, title: string, topdf: boolean, openinwindow: boolean }) => {
        ...
        if (openinwindow) {
            let url = '/api/platform/dataobjectexport/exportexcelscheme.do?' + urlEncode(params);
            const windowtitle = selectedTextValue[0].text +
            (selectedRowKeys.length == 1 ? '' : '等' + selectedTextValue.length + '条记录') + '的' + title;
            const pdfjsurl = getPdfjsUrl(url,windowtitle + '.pdf');
            onOpenInNewWindow(pdfjsurl, windowtitle, 'pdf')
        } else
            downloadRecordExcel(params);
    }
