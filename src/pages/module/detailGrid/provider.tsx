...
import { ParentFilterModal, ParentFormModal } from '../data';
import { getDefaultModuleState } from '../modules';
import { moduleStateReducer, DetailModelContext, wrapperDispatch, wrapperSelectDispatch } from './model';
...
export const DetailModelProvider: React.FC<DetailModelProviderProps> =
    ({ moduleName, parentFilter, parentForm, children }) => {
        const [moduleState, dispatch] = useReducer(moduleStateReducer,
            getDefaultModuleState({ moduleName, parentFilter, parentForm }));
        return (
            <DetailModelContext.Provider value={{
                moduleState,
