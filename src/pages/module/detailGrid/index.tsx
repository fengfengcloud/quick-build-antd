...
const DetailGrid: React.FC<DetailGridPrpos> = ({ moduleName, parentOperateType, parentFilter,
    readOnly, enableUserFilter = false, parentForm }) => {
    // const title = '『' + parentFilter.fieldtitle + ':'
    //     + (parentFilter.text && parentFilter.fieldvalue ? parentFilter.text : '未选中') + '』 ';
    return (
        <DetailModelProvider moduleName={moduleName}
            parentFilter={parentFilter}
            parentForm={parentForm}>
            <DetailTable pFilter={parentFilter}
...
const DetailTable = ({ pFilter, readOnly, enableUserFilter }:
    { pFilter: ParentFilterModal, readOnly: boolean, enableUserFilter: boolean }) => {
    const context = useContext(DetailModelContext);
    console.log(context);
    const { dispatch } = context;
    const moduleState = context.moduleState as ModuleState;
    const { moduleName, filters: { parentfilter: parentFilter } } = moduleState;
