...
import { getAllFilterAjaxParam, getColumnFiltersInfo, getGridColumnFilters } from '../grid/filterUtils';
...
export const wrapperDispatch = (moduleState: ModuleState, dispatch: any) => {
    return function (action: ActionProps) {
        ...
        if (type == 'fetchData') {
            if (moduleState.filters.parentfilter?.fieldvalue) {
                ...
                fetchDataAsync(moduleState).then((response: any) => {
                    dispatch({
                        type: 'updateDataList',
                        payload: response,
                    })
...
export const DetailModelContext = createContext<ModuleStateContext>({ moduleState: undefined, dispatch: undefined });
...
const resetSelectedRow = (moduleState: ModuleState, action: ActionProps) => {
    return {
        ...moduleState,
        selectedRowKeys: [],
        selectedTextValue: [],
...
const insertRecord = (moduleState: ModuleState, action: ActionProps) => {
    ...
    const dataSource = [...moduleState.dataSource, record].reverse().
...
// 恢复到默认排序
const resetSorts = (moduleState: ModuleState, action: ActionProps) => {
...
const updateDateList = (moduleState: ModuleState, action: ActionProps) => {
    ...
    const { curpage, limit, start, total, totalpage, data: dataSource } = action.payload;
    // 把不是当页的选中的记录全部清除掉。默认刷新过后，不保留非当前页的选中记录
    // 树形结构的只保留根结点的选中状态
    const selectedRowKeys = moduleState.selectedRowKeys.filter((key: any) =>
        dataSource.find((record: any) =>
            record[primarykey] == key) !== undefined)
    return {
        ...moduleState,
...
const fetchDataAsync = (moduleState: ModuleState) => {
    ...
    apply(payload, getAllFilterAjaxParam(filters));
    if (sorts.length) {
