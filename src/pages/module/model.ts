...
import {
    setModuleInfo, generateModuleInfo, hasModuleInfo, getModuleInfo,
        ...
        *filterChanged({ payload }, { call, put, select }) {
            ...
            switch (type) {
                ...
                case 'parentFilterChange':
                    ...
                    filters.searchfilter = undefined;                       // 查询框中的文字
                ...
                case 'resetSqlparam':
                    ...
                    for (let key in filters.sqlparam) {
    ...
    reducers: {
        ...
        removeAttachment(state = {}, action) {
            ...
            const dataSource = moduleState.dataSource.map(rec => {
                if (rec[primarykey] === recordId) {
                    ...
                    updatedRecord.attachmentdata = updatedRecord.attachmentdata.filter((record: any) => record.id !== attachmentId);
        ...
        parentFilterChanged(state = {}, action) {
            ...
            const filters: ModuleFilters = {
                ...
                searchfilter: undefined,                       // 查询框中的文字
        ...
        insertRecord(state = {}, action) {
            ...
            if (moduleInfo.istreemodel) {
                dataSource = addRecordToTree(moduleState.dataSource, record, moduleInfo.primarykey, moduleInfo.codelevel || '');
            } else {
                record.__recno__ = '+';
                // 如果超过当前页的显示记录数，则不会显示出来了
                dataSource = [...moduleState.dataSource, record].reverse().
                    filter((_, index) => index < gridParams.limit).reverse();
            }
        ...
        updateRecord(state = {}, action) {
            ...
            let formState = moduleState.formState;
        ...
        updateDataList(state = {}, action) {
            ...
            const { curpage, limit, start, total, totalpage, expandedRowKeys, remoteRoot } = action.payload;

            // 把不是当页的选中的记录全部清除掉。默认刷新过后，不保留非当前页的选中记录
            // 树形结构的只保留根结点的选中状态
            const allRecord = moduleInfo.istreemodel ? getAllTreeRecord(dataSource) : dataSource;
            const selectedRowKeys = moduleState.selectedRowKeys.filter((key: any) =>
                allRecord.find((record: any) =>
            ...
            result[moduleName] = {
                ...
                remoteRoot: remoteRoot || {},
                selectedRowKeys,
                expandedRowKeys: moduleInfo.istreemodel ? moduleState.expandedRowKeys.length ?
                    moduleState.expandedRowKeys : expandedRowKeys : moduleState.expandedRowKeys,
        ...
        selectedRowKeysChanged(state = {}, action) {
            ...
            const allRecord = moduleInfo.istreemodel ?
                getAllTreeRecord(moduleState.dataSource) : moduleState.dataSource;
        ...
        expandChanged(state = {}, action) {
            ...
            const result = { ...state };
            if (selected) {          // 展开即选中，不是树都这样操作，只能展开一个
                result[moduleName] = {
                    ...moduleState,
        ...
        monetaryChanged(state = {}, action) {
            ...
            if (position) {
                ...
                setDefaultMonetaryPosition(moduleState.moduleName, position);
            }
            if (monetaryType) {
                result[moduleName] = { ...moduleState, monetary: getMonetary(monetaryType) };
                setDefaultMonetaryType(moduleState.moduleName, monetaryType);
            }
            return result;
        },
