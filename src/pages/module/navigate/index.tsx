...
import {
    ReloadOutlined, NodeExpandOutlined, NodeCollapseOutlined, LinkOutlined, DisconnectOutlined,
...
const getTitle = (node: any, search: string = '') =>
    <>{search ? <Highlighter highlightClassName="ant-btn-link"
        searchWords={[search]} textToHighlight={node.title} /> :
        (node.title || '未定义')}
...
const genMiniTreeData = (nodes: any[]): any => {
    const result: any[] = [];
    result.push(...nodes.filter((_, index: number) => index < LEVELMAXCOUNT).
        map((node: any) => {
            const { children } = node;
...
const genSearchedPingYingTreeData = (nodes: any[], search: string, expandedKeys: string[], matchedCount: number[]): any[] => {
    const result: any[] = [];
    nodes.forEach((node: any) => {
        const find = match(node.title, search);
        if (find) matchedCount[0]++;
        ...
        if (node.isLeaf) {
            ...
            const findChildren: any[] = genSearchedPingYingTreeData(node.children || [], search, expandedKeys, matchedCount);
...
const genSearchedTreeData = (nodes: any[], search: string, expandedKeys: string[], matchedCount: number[]): any[] => {
    const result: any[] = [];
    nodes.forEach((node: any) => {
        ...
        if (node.isLeaf) {
            ...
            const findChildren: any[] = genSearchedTreeData(node.children || [], search, expandedKeys, matchedCount);
            if (find || findChildren.length) {
                if (findChildren.length) {
                    treeNode.children = findChildren;
                    expandedKeys.push(node.key);
...
// 一个模块的导航区域
const Navigate = ({ moduleState, dispatch }:
    ...
    if (moduleState.filters.sqlparam) {
        nameAndParent = nameAndParent + '-' + JSON.stringify(getSqlparamFilter(moduleState.filters.sqlparam));
    }
    // 第一次加载，需要把所有的导航信息生成放在本模块的变量里
    if (!moduleNavigates[nameAndParent]) {
        moduleNavigates[nameAndParent] =
    ...
    if (schemes != moduleNavigates[nameAndParent])
        setSchemes(moduleNavigates[nameAndParent]);
    ...
    const addChildrenDataToNode = (scheme: NavigateStateModal, node: any): any[] => {
        ...
        const { search } = scheme;
        if (search)
            isletter = /^[a-zA-Z]+$/.test(search);
        // 找到node,在 treeData中的记录
        const cloneNode = (nodes: any[]) => {
            return nodes.map((n_: any) => {
                const n = { ...n_ };
                if (n.key == node.key) {
                    n.children = childrenSource.filter((_, index: number) => index < EXPANDLEVELMAXCOUNT).
                        map((item: any) => {
                            if (isletter) {
    ...
    const getTree = (scheme: NavigateStateModal) => {
        return <DirectoryTree blockNode
            style={{ whiteSpace: 'nowrap' }}
            expandAction='doubleClick'
            onSelect={(selectedKeys: any, e: any) =>
                onNavigateSelected(selectedKeys, e, nameAndParent + scheme.navigateschemeid)}
            ...
            onExpand={(expandedKeys: any, { expanded, node }) => {
                ...
                if (expanded && (!node.children ||
                    ...
                    setSchemes(updateTo(nameAndParent, { ...scheme, expandedKeys: expandedKeys, treeData }))
                } else
                    setSchemes(updateTo(nameAndParent, { ...scheme, expandedKeys: expandedKeys }))
            }}
    ...
    const buildNavigateData = (scheme: NavigateStateModal) => {
        ...
        const params: any = {
            moduleName, title, navigateschemeid, cascading, isContainNullRecord,
        ...
        fetchNavigateTreeData(params).then((data) => {
            const expandedKeys: any[] = [];
            const canExpandedKeys: any[] = [];
            let key = 1000;
            const rebuildData = (item: any) => {
            ...
            if (children && children.length > 0)
                children[0].expanded = true; //root展开
            ...
            let params: NavigateStateModal = {
                ...scheme, expandedKeys: initExpandedKeys,
                canExpandedKeys, dataSource, dataSourceKeyIndex,
                loading: 'loaded', nodeCount, treeData, initExpandedKeys, initTreeData
            }
            // 如果改变datasource之后有搜索值，则把搜索条件加上
            if (scheme.search)
                params = { ...params, ...genSearchedDataAndExpandedKeys(dataSource, scheme.search) }
            setSchemes(updateTo(nameAndParent, params));
        })
    }

    const getTool = (scheme: NavigateStateModal) =>
        //如果是单层导航，并且节点少于15个，并且没有显示无记录导航的按钮，那就整个不显示
        scheme.allLevel === 1 && scheme.nodeCount <= LEVELMAXCOUNT && !scheme.allowNullRecordButton ? null :
            <Input.Group size="small" style={{ display: 'flex', paddingBottom: 3 }}>
                <Space >
                    ...
                    /></Tooltip>
                    <Tooltip title="子级全部折叠"><NodeCollapseOutlined style={{ cursor: 'pointer' }}
                        onClick={() => {
                            const expandedKeys = scheme.treeData.filter((node) => !node.isLeaf).map((node) => node.key);
                            setSchemes(updateTo(nameAndParent, { ...scheme, expandedKeys }))
                        }} /></Tooltip>
                    {scheme.allLevel > 1 ?
                        (scheme.cascading ?
                ...
                <Search size="middle" placeholder="搜索文本或简拼或全拼" style={{ flex: 1 }}
                    ...
                    onSearch={(search: string) => {
                        if (search) {
                            ...
                            setSchemes(updateTo(nameAndParent, {
                                ...scheme, search,
    ...
    if (schemes.length > 1)
        return (
            <Card title="导航" bordered={false}
                bodyStyle={{ overflowY: 'auto' }}
                extra={getExtra()} size="small" >
                <Tabs className="navigate"
        ...
        if (scheme.loading === 'loaded')
            return <Card bordered={false} bodyStyle={{ overflowY: 'auto' }}
