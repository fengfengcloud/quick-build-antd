    ...
    <Form className="moduleform" autoComplete="off" form={form}>
      <Row gutter={16}>
        ...
        <Col xs={24} key={APPROVECONTEXT}>
          <FormItem label="审批意见" name={APPROVECONTEXT} labelCol={labelCol} key={APPROVECONTEXT}>
                ...
                {(thisform: any) => {
                  return (
                    <Popconfirm
                      ...
                      onConfirm={() => {
                        const submitData = thisform.getFieldsValue();
                        submitApprove({ submitData, button });
                      }}
                    >
                      <Button type={index === 0 ? 'primary' : 'default'}>{button.name}</Button>
                    </Popconfirm>
                  );
                }}
              </Form.Item>
