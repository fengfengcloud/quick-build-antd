...
let scale = 1;
const setScale = (n: number) => scale = n;
export const activitiModeler = ({ record, dispatch }:
    { record: any, dispatch: Dispatch }) => {
    ...
    const download = (type: string, data: any, name: string) => {
        let dataTrack = type == 'xml' ? 'bpmn' : 'svg';
        const a = document.createElement('a');
        name = name || `${record.name}的流程.${dataTrack}`;
    ...
    setGlobalModalProps({
        onCancel: () => setGlobalModalProps((props: any) => ({ visible: false })),
        destroyOnClose: true,
        visible: true,
        centered: true,
        okText: '保存',
        ...
        children: (
            <div>
                <div style={{ height: (document.documentElement.clientHeight - 56 - 52) + 'px' }} className="content with-diagram" >
