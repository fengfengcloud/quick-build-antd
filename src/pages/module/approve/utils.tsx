...
import {
    QuestionCircleOutlined, PlayCircleOutlined, CheckCircleOutlined, ClockCircleOutlined,
    AuditOutlined, StopOutlined, CheckOutlined, InfoCircleOutlined, PartitionOutlined,
    UndoOutlined
 ...
 * @param param0 
 */
const getCanStartPopover = ({ moduleInfo, record, className, dispatch }:
    { moduleInfo: ModuleModal, record: any, className: string, dispatch: Dispatch }) => {
    const text = "『" + record[moduleInfo.namefield] + '』';
    const state = '审批流程尚未启动';
    const start = () => startProcess(moduleInfo, record, dispatch);
    return canStartProcess(moduleInfo) ?
        <Popover content={
            <span>{state}
            ...
            <Popconfirm
                ...
                onConfirm={start}
            >
                <a><span className={className} ></span></a >
            </Popconfirm>
        </Popover> :
 ...
 * @param param0 
 */
const getCanClaimPopover = ({ moduleName, record, className, tips, dispatch }:
    { moduleName: string, record: any, className: string, tips: any, dispatch: Dispatch }) => {
    return (
 ...
 * @param param0 
 */
const getCanProcessPopover = ({ moduleState, record, className, tips, dispatch }:
    { moduleState: ModuleState, record: any, className: string, tips: any, dispatch: Dispatch }) => {
    return (
        <Popover content={tips}>
...
const getSVGFromProcDefId = (procDefId: string): string => {
    if (!procDefIdSVGMap.has(procDefId)) {
        procDefIdSVGMap.set(procDefId, syncRequest(`/api/platform/workflowdesign/getsvg.do`,
            { params: { procDefId } }))
    }
    return procDefIdSVGMap.get(procDefId) || '';
}

export const approveRenderer: React.FC<ApproveRenderProps> = ({ value = [], record,
    moduleState, dispatch, isLink = true, readonly = false, _recno }) => {
    ...
    const tips =
        <Card title={<><AuditOutlined /> 流程审批记录</>}
            extra={<Space>
                {<Tooltip title="显示审批流程图" placement="bottom">
                    <Popover title={<Space>
                        <AuditOutlined />{`${moduleInfo.title}的审批流程图`}
                    </Space>} content={
                ...
                {moduleInfo.userLimit.approve?.cancel ?
                    <Tooltip title="取消当前记录的所有审批信息" placement="bottom">
                        <Popconfirm
                            icon={<QuestionCircleOutlined style={{ color: 'red' }} />}
    ...
    if (isFinishProcess(record)) {
        return <Popover placement="rightTop" trigger='hover'
            ...
            {approveEndState(record) == 'approve' ?
                <CheckOutlined /> : <span style={{ color: 'red' }}><StopOutlined /></span>}
        </Popover >
    }
    return <Popover placement="rightTop" trigger='hover'
        content={tips}>
...
// 我的待办事项里面的记录的审批
export const approveVActRuTaskRenderer: React.FC<ApproveRenderProps> = ({ record,
    ...
    const tips =
        <Card title={<><AuditOutlined /> 流程审批记录</>}
            extra={<Tooltip title="显示审批流程图" placement="bottom">
                <Popover title={<Space>
                    <AuditOutlined />{`${moduleInfo.title}的审批流程图`}
                </Space>} content={
                    <div><span dangerouslySetInnerHTML={{
    ...
    if (canApprove(record)) {
        const CreatePaymentButton = () => {
            ...
            return <React.Fragment>
                <Popover content={tips}>
                    <a key={'button-' + record['actExecuteTaskId']}
                        onClick={() => {
...
export const getApproveIconClass = (moduleInfo: ModuleModal, record: any): string => {
    if (!isStartProcess(record)) {
        if (canStartProcess(moduleInfo))
            return 'approveaction x-fa fa-play fa-fw';      // 可以启动
        else
            return 'actionyellow x-fa fa-exclamation-triangle fa-fw'        //不能启动
    }
    if (canClaim(record))
        return 'approveaction x-fa fa-inbox fa-fw'; // 可以拾取
    if (canApprove(record))  // 有当前用户的任务id,并且是当前人员,即是当前用户可以审批
    ...
    if (record['actProcState']) {
        ...
        else if (record['actProcState'] == '已暂停') return 'approvepause fa-fw'
        else if (record['actProcState'].indexOf('已结束') == 0) {
...
export const isApproveFromActionName = (name: string) => {
    return !(name.indexOf('不') >= 0 || name.indexOf('暂') >= 0 || name.indexOf('终') >= 0 || name.indexOf('退') >= 0)
}
 ...
 * @param moduleInfo 
 */
export const canStartProcess = (moduleInfo: ModuleModal): boolean => {
    return !!moduleInfo.userLimit.approve && moduleInfo.userLimit.approve.start;
}
...
const canClaim = (record: any) => {
    // 审批人员为空的情况下，在候选人名单中有
    return !isPauseProcess(record) && (!record['actAssignee'] && find_in_set(currentUser.userid, record['actCandidate']));
}
...
const canUnClaim = (record: any) => {
    ...
    return !isPauseProcess(record)
        && (record['actAssignee'] == currentUser.userid && find_in_set(currentUser.userid, record['actCandidate']));
}
...
const { Step } = Steps;
export const getApproveSteps = ({ moduleState, record, dispatch, direction = 'horizontal', readonly = false }:
    { moduleState?: ModuleState, record: any, dispatch?: Dispatch, direction?: 'horizontal' | 'vertical', readonly?: boolean }) => {
    const steps: any[] = [];
    if (!record || !isStartProcess(record)) return <span>审批流程尚未启动</span>;
    steps.push(<Step title='流程启动' key="process_start"
        icon={<PlayCircleOutlined />}
        subTitle=''
        status='finish'
        description={<span>{(record['actStartUser.username'] || record['actStartUserName'])
            + '(' + deleteSecond(record['actStartTime']) + ')'} </span>} >
    </Step>)
    steps.push(...record['actCompleteTaskInfo'].map((task: ApproveTask) => {
        return <Step title={task.taskname} key={task.taskname}
            subTitle={task.processresult}
            status={isApproveFromActionName(task.processresult) ? 'finish' : 'error'}
            description={<span>{task.username + '(' + deleteSecond(task.processdate) + ')'} <br />
                {task.processtitle && task.processtitle.length > MAXTITLELENGTH && direction === 'horizontal' ?
                    <Tooltip title={task.processtitle}><span><InfoCircleOutlined /> 审批意见</span></Tooltip> :
                    <span style={{ maxWidth: '300px', display: 'block' }}>{task.processtitle}</span>
                }</span>} >
        </Step>
    }))
    if (isFinishProcess(record)) {
        steps.push(<Step title={record['actEndActName']} key="process_finished"
            icon={approveEndState(record) == 'approve' ?
                <CheckCircleOutlined /> : <span style={{ color: 'red' }}><StopOutlined /></span>}
            status='finish'
            description={<span>{'审批结束'
                + '(' + deleteSecond(record['actEndTime']) + ')'} </span>} >
        </Step>)
    } else {
        const cad = record['actCurrentCandidateName'];
        // 如果没传dispatch和moduleState进来，那么就不能进行操作
        if (canClaim(record) && dispatch && moduleState) {
            steps.push(<Step title={record.actTaskNames}    //'等待接受任务'
                key="process_claim"
                icon={<ClockCircleOutlined />}
                status='process'
                description={
                    <span>{cad ? <>{'正在等待 ' + cad + ' '}
                        <a onClick={() => claimOrUnClaim(moduleState.moduleName, record, dispatch)}>接受任务</a></> : ''}
                    </span>} >
            </Step>)
        } else if (canApprove(record) && dispatch && moduleState) { // 有当前用户的任务id
            steps.push(<Step title={record.actTaskNames}  //'等待处理任务'
                icon={<ClockCircleOutlined />}
                key="process_process"
                status='process'
                description={<span>正在等待 我 对任务进行处理
                    {readonly ? null : <a onClick={() => executeProcess(moduleState, record, dispatch)}
                        style={{ marginLeft: '24px' }}>开始审批</a>}
                    {// 记录刷新之后，就无法退回任务了,接受任务之后，在当前记录不刷新的情况下可以退回任务
                        canUnClaim(record) ?
                            <a style={{ marginLeft: '12px' }}
                                onClick={() => claimOrUnClaim(moduleState.moduleName, record, dispatch)}>退回任务</a> : null}
                </span>} >
            </Step>)
        } else if (isInProcess(record)) {
            const ass = record['actCurrentAssignName'];
            const dotext = cad ? '接受任务' : '处理任务';
            steps.push(<Step title={record.actTaskNames} //{`等待${dotext}`}
                key={"process_" + cad}
                status='process'
                icon={<ClockCircleOutlined />}
                description={<span>{(cad ? '正在等待 ' + cad + ' ' + dotext : '')
                    + (ass ? '正在等待 ' + ass + ' ' + dotext : '')}</span>} >
            </Step>)
        }
    }
    return <Steps size="small" direction={direction} >{steps}</Steps>
}
...
const claimOrUnClaim = (moduleName: string, record: any, dispatch: Dispatch) => {
    ...
    if (canClaim(record) || canUnClaim(record)) {
        if (canUnClaim(record)) { // 已经指定了办理人，就可以退回
            act = 'unclaim';
            text = '退回';
        }
        request('/api/platform/workflow/runtime/' + act + '.do', {
            ...
            if (result.success) {
                ...
                Modal.warning({
                    ...
                    content: <span dangerouslySetInnerHTML={{ __html: result.msg }}></span>,
                });
            }
        })
    }
...
*/
const find_in_set = (str: string = '', liststr: string = '') => {
    if (liststr) {
        const listarray = liststr.split(',');
