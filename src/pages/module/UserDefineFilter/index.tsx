...
import { getBooleanFilterOption, getBooleanInValueText, NumberFilterSelectOption } from '../grid/filterUtils';
...
export const setFilterInitValues = (moduleName: string, initFilter: any) => {
    if (!filterInitValues[moduleName])
        filterInitValues[moduleName] = {};
...
export const initUserFilterFieldInitValues = (moduleName: string, scheme: any) => {
    if (!scheme || !scheme.details[0] || !scheme.details[0].details)
        return;
    scheme.details[0].details.forEach((filterField: any) => {
        ...
        if (filterField.comboThisField) { }
        else if (filterField.fDictionaryid)
            apply(filter, { fDictionaryid: filterField.fDictionaryid, });
        else if (filterField.isNumberField)
            apply(filter, { operator: '=', type: 'number', });
        ...
        else if (filterField.isBooleanField)
            apply(filter, { type: 'boolean', });
            ...
            apply(filter, { operator: 'like', type: 'string', })
...
const UserDefineFilter = ({ moduleState, dispatch, visible }:
    { moduleState: ModuleState | any, dispatch: any, visible: boolean }) => {
    const { moduleName, currSetting: { userFilterRestHidden, userFilterRestNumber } } = moduleState;
    ...
    const getACol = (filterField: any, cols: number): any => {
        ...
        return <Col xs={24} key={filterField.detailid}
            ...
            {filterField.comboThisField ? getComboThisFieldFilter(params) :
                filterField.fDictionaryid ? getDictionaryFilter(params) :
                    filterField.isNumberField ? getNumberFilter(params) :
                        filterField.isDateField ? getDateFilter(params) :
                            filterField.isBooleanField ? getBooleanFilter(params) :
                                filterField.xtype == 'usermanytoonetreefilter' ? getManyToOneTreeFilter(params) :
    ...
    const getButtonRegion = () => {
        return <span style={{ display: 'flex', marginBottom: '6px', marginTop: '6px' }}>
            ...
            {userFilterRestNumber !== -1 ?
                <Button type="link"
                    ...
                    {userFilterRestHidden ? <DownOutlined /> : <UpOutlined />}</Button> :
                <Button type="link"
                    onClick={() => {
                        dispatch({
    ...
    const getFilterForm = (scheme: any, hidden: boolean) => {
        const cols = scheme.details[0].cols || 3;
        let count = scheme.details[0].details.length;
        if (userFilterRestNumber !== -1 && hidden)
            count = userFilterRestNumber;
        const fieldsArray: any[] = [];
        const hiddenArray: any[] = [];
        for (let i = 0; i < count; i++) {
            fieldsArray.push(getACol(scheme.details[0].details[i], cols));
        }
        for (let i = count; i < scheme.details[0].details.length; i++) {
            hiddenArray.push(getACol(scheme.details[0].details[i], cols));
        }
        return <Row gutter={0}>
            {fieldsArray}
    ...
    return (
        <Card
            style={{ marginBottom: 16, display: visible ? 'inherit' : 'none' }}
            bordered={true} >
                ...
                <Form.Item name={[filterField.fieldname, 'value']} noStyle>
                    <TagSelect hideCheckAll={!!filterField.hideCheckAll} expandable={filterField.expandable}
                        ...
                        {dictData.map((rec: TextValue) =>
                            <TagSelect.Option key={rec.value} value={rec.value}>{rec.text}</TagSelect.Option>)}
                    </TagSelect>
                </Form.Item>
                        ...
                        <Form.Item name={[filterField.fieldname, 'value']} noStyle>
                            <Select mode="multiple" style={{ flex: 1 }} allowClear optionFilterProp="label">
                                {dictData.map((rec: any) =>
                                    <Option key={rec.value} value={rec.value} label={rec.label}>{rec.text}</Option>)}
                            </Select>
                        </Form.Item>
                        <Form.Item name={[filterField.fieldname, 'operator']} noStyle >
                            <Input type="hidden" />
 ...
 * @param param0 
 */
const getComboThisFieldFilter: React.FC<UserFilterProps> = ({ moduleState, filterField, labelWarrapCol }): any => {
...
const getDictionaryFilter: React.FC<UserFilterProps> = ({ moduleState, filterField, labelWarrapCol }): any => {
    let dictData: TextValue[] = getDictionaryData(filterField.fDictionaryid).
...
const getManyToOneFilter: React.FC<UserFilterProps> = ({ moduleState, filterField, labelWarrapCol }): any => {
    let dictData: TextValue[] =
        getModuleComboDataSource(filterField.manyToOneInfo.objectname).
            map((rec: any) => ({ ...rec, label: rec.text }));
    dictData = addCountToText(dictData, moduleState, filterField);
    if (filterField.allowEmpty)
        dictData.splice(0, 0, { value: 'null', text: '未定义', label: '未定义' })
...
const getManyToOneTreeFilter: React.FC<UserFilterProps> = ({ filterField, labelWarrapCol }): any => {
    const title = filterField.title || filterField.defaulttitle;
    const dictData: TextValue[] = getModulTreeDataSource(filterField.manyToOneInfo.objectname, true,
        ...
        <Input.Group compact style={{ display: 'flex' }}>
            <Form.Item name={[filterField.fieldname, 'value']} noStyle>
                <TreeSelect style={{ flex: 1 }} allowClear
                    showCheckedStrategy={SHOW_PARENT} showSearch treeNodeFilterProp="label"
                    treeData={options} treeCheckable={true} treeDefaultExpandAll={true}>
                </TreeSelect>
            </Form.Item>
            <Form.Item name={[filterField.fieldname, 'operator']} noStyle >
                <Input type="hidden" />
...
const getBooleanFilter: React.FC<UserFilterProps> = ({ moduleState, filterField, labelWarrapCol }): any => {
            ...
            >
                <Select style={{ width: OPERATEWIDTH }}>
                    {NumberFilterSelectOption.map((idtext: any) =>
                        <Option key={idtext.value} value={idtext.value}>{idtext.text}</Option>)}
                </Select>
            </Form.Item>
            <Form.Item
