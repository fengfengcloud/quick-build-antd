...
import { TextValue, ModuleState } from '../data';
...
export const getDateFilter = ({ moduleState, filterField, form, labelWarrapCol }:
    ...
    if (addText) {
        ...
        if (date)
            return moment(date).format(type == 'year' ? 'YYYY' : type == 'month' ?
                'YYYY-M' : type == 'quarter' ? 'YYYY-Q' : type == 'week' ? 'YYYY-w' : DateFormat);
        else
            return '';
    }
}
