...
const getLevel = (code: string, codelevel: string[]): number => {
    ...
    for (let i = 0; i < codelevel.length; i++) {
        if (strlen === getLevelLen(i + 1, codelevel))
            return i + 1;
    }
    return -1;
}
...
export const getPinRecord = (records: any[], pinkey: string, primarykey: string): any => {
    ...
    const findPinRecord = (recs: any[]) => {
        recs.forEach((rec: any) => {
            ...
            if (!result && rec.children && Array.isArray(rec.children))
                findPinRecord(rec.children);
        })
    }
...
export const getAllTreeRecord = (records: any[]) => {
    ...
    const getThislevel = (record: any) => {
        ...
        if (record.children && record.children.length)
            record.children.forEach((record: any) => getThislevel(record));
    }
...
export const getMaxPrimaryKeyFromKey = (data: any[], key: string, primarykey: string): string => {
    ...
    if (!parent) {
        ...
        return (parent.children as any[]).reduce((a, b) => b[primarykey] > a[primarykey] ? b : a)[primarykey];
    }
}

export const getRecordByKey = (dataSource: any[], key: string, primarykey: string) => {
    for (let i in dataSource) {
        if (dataSource[i][primarykey] == key)
            return dataSource[i];
    }
    return null;
}
...
// 
export const isParentFilterChanged = ({ moduleState, parentFilter: newpf }: {
    moduleState: ModuleState, parentFilter: ParentFilterModal | undefined
}) => {
    ...
    if (mpf) {
        if (!newpf)
            ...
            return mpf.moduleName !== newpf.moduleName || mpf.fieldahead !== newpf.fieldahead
                || mpf.fieldvalue !== newpf.fieldvalue || mpf.fieldName !== newpf.fieldName
 ...
 * @param pf 
 */
export const getParentFilterTitle = (pf: ParentFilterModal | undefined, moduleName: string) => {
    return pf ?
        <span className={styles.headerparenttext}>
            ...
            <Tooltip title="取消限定条件">
                <span>
                    ...
                    }}><CloseOutlined style={{ paddingLeft: '6px' }} /></a>
                </span>
            </Tooltip>
        </span> :
        <></>
...
export const getModuleIcon = (moduleInfo: ModuleModal) => {
    return moduleInfo.iconcls ? <span className={moduleInfo.iconcls}></span> : <></>
}
