...
const preview: React.FC<AttachmentPreviewProps> = ({ location }) => {
    let attachmentid = location.query.attachmentid;
    let saveName = location.query.saveName;
    if (!attachmentid) {
    ...
    const url = url_ + `/preview.do?attachmentid=${attachmentid}`;
    // 使用pdfjs的  pdfjsurl
...
export const attachmentPdfOpenPreview = (attachmentid: string, saveName: string) => {
    ...
    sessionStorage.setItem('saveName', saveName);
