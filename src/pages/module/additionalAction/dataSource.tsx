...
export const testDataSource = (params: ActionParamsModal) => {
    ...
    let datasourceid = record[moduleInfo.primarykey];
    request('/api/platform/datasource/testconnect.do', {
        ...
        if (response.success) {
            Modal.info({
                width: 500,
                title: '数据源连接测试成功',
                content: <span dangerouslySetInnerHTML={{ __html: '连接字符串：' + response.tag }}></span>,
            })
        } else {
            Modal.error({
                width: 500,
...
export const importSchema = (params: ActionParamsModal) => {
    ...
    const ImportForm = () => {
        ...
        return <Form form={form} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} autoComplete='off'>
            <Form.Item label='业务数据库名' name='name'
                rules={[{ required: true }]}>
                <Select dropdownStyle={{ zIndex: 10001 }}
                    onChange={(value) => {
                        form.setFieldsValue({ title: value })
                    }}>
                    {schemes.map(scheme => <Select.Option key={scheme} value={scheme}>{scheme}</Select.Option>)}
                </Select>
            </Form.Item>
            <Form.Item label='表对象前缀' name='objectnameahead'
                // regexg 不起作用？
                rules={[{ type: 'regexp', pattern: /^[A-Za-z][\w]{0,5}$/, message: '字母开头,只包含字母数字下划线' },
                { required: true }]}
            ...
            <Form.Item label='数据库说明' name='title' help='详细描述此业务数据库的用途等信息'
                rules={[{ required: true }]}>
                <Input maxLength={50} />
            </Form.Item>
                ...
                <Button type='primary' onClick={() => {
                    form.validateFields().then(value => {
                        ...
                        if (!/^[A-Za-z][\w]{0,5}$/.test(values.objectnameahead)) {
                            setFieldsValidate({ objectnameahead: '表对象前缀必须以字母开头,只包含字母数字下划线。' })
                            return;
                        }
                        request('/api/platform/datasource/addschema.do', {
                            ...
                            if (result.success) {
                                message.info('业务数据库『' + values.title + '』' + '导入成功!,请去业务数据库模块中导入表。');
