...
import { queryUserAllLimits, queryUserLimits, saveUserLimits, queryRoleLimits, saveRoleLimits } from './systemActionService';
...
const DisplayUserLimits: React.FC<any> = ({ userid, timestramp }:
    { userid: string, timestramp: number }) => {
    ...
    useEffect(() => {
        ...
        queryUserAllLimits({ userid }).then((limitData) => {
            ...
            const columns: any = [titleColumns];
...
const SetUserLimits: React.FC<any> = ({ userid, msg, timestramp }:
    { userid: string, msg: string, timestramp: number }) => {
    ...
    useEffect(() => {
        ...
        queryUserLimits({ userid }).then((limitData) => {
            let key = 1;
            const keys: string[] = []
            const adjust = function (record: any) {
                record.title = record.text;
                if (record.objectid)
                    record.key = record.objectid;
                else
                    record.key = 'key-' + key++;
                if (record.checked)
                    keys.push(record.key)
                record.children && record.children.forEach((r: any) => {
                    adjust(r);
                })
            }
            // 所有展开的行的key
    ...
    const checkall = () => {
        ...
        const addtokeys = (record: any) => {
            keys.push(record.key);
            record.children && record.children.forEach((rec: any) => addtokeys(rec))
        }
    ...
    const checkquery = () => {
        ...
        const addtokeys = (record: any) => {
            const { type } = record;
            if (type == 'query' || type == 'attachmentquery')
                keys.push(record.key);
            record.children && record.children.forEach((rec: any) => addtokeys(rec))
        }
        data.forEach((record: any) => addtokeys(record));
    ...
    return <div style={{ height: '100%', display: 'flex', flexFlow: 'column' }}>
        <div style={{ display: 'flex' }}>
            ...
            <Button icon={<SaveOutlined />} type="primary" onClick={save} disabled={!changed}>保存</Button>
        </div>
...
const SetRoleLimits: React.FC<any> = ({ roleid, msg, display, timestramp }:
    { roleid: string, msg: string, display: boolean, timestramp: number }) => {
    const [changed, setChanged] = useState<boolean>(false);
    const [data, setData] = useState([]);
    const [checkedKeys, setCheckedKeys] = useState<string[]>([]);
    const [expandedKeys, setExpandedKeys] = useState<string[]>([]);
    ...
    return <div style={{ height: '100%', display: 'flex', flexFlow: 'column' }}>
        {display ? null : <div style={{ display: 'flex' }}>
            <Space style={{ marginBottom: '16px' }}>
                <Button onClick={checkall}>全选</Button>
                <Button onClick={checkquery}>仅浏览</Button>
                <Button onClick={reset}>重置</Button>
