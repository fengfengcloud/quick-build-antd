...
const resetUserPassword = (params: ActionParamsModal) => {
    const { record, moduleInfo: { primarykey, namefield } } = params;
    const username = record[namefield];
    Modal.confirm({
        title: `确定要重置用户『${username}』的密码吗?`,
        icon: <ExclamationCircleOutlined />,
        onOk() {
            queryResetUserPassword({ userid: record[primarykey] })
                .then((response: any) => {
                    if (response.success)
                        message.success(`用户『${username}』的密码已重置为“123456”请通知其尽快修改!`);
                    else
                        message.error(`用户『${username}』的密码已重置失败!`);
                })
        }
    });
}
...
const createPersonnelUser = (params: ActionParamsModal) => {
    const { record, moduleInfo: { primarykey, namefield, modulename: moduleName }, dispatch } = params;
            ...
            Modal.info({
                ...
                content: <span
                    dangerouslySetInnerHTML={{
                        __html: '<br/>' + response.message +
                            '<br/><br/>请尽快通知其修改密码，并在用户模块中给其设置权限！'
                    }}></span>
            })
...
const displayUserLimit = (params: ActionParamsModal) => {
    const { record, moduleInfo: { primarykey, namefield }, funcDefine } = params;
    const props: UserDrawerProps = {
        visible: true,
        title: <><span className={funcDefine.iconcls}></span>{` 用户『${record[namefield]}』的操作权限`}</>,
        width: '60%',
        children: <DisplayUserLimits userid={record[primarykey]} timestramp={new Date().getTime()} />,
        onClose: () => setGlobalDrawerProps((props: any) => ({ ...props, visible: false })),
    }
...
const setUserLimit = (params: ActionParamsModal) => {
    ...
    const props: UserDrawerProps = {
        visible: true,
        title: <><span className={funcDefine.iconcls}></span> {msg}</>,
        width: '500px',
        children: <>
...
const displayRoleLimit = (params: ActionParamsModal) => {
    ...
    const props: UserDrawerProps = {
        ...
        title: <><span className={funcDefine.iconcls}></span> {msg}</>,
        width: '500px',
        children: <SetRoleLimits display={true}
            roleid={record[primarykey]} msg={msg} timestramp={new Date().getTime()} />,
        onClose: () => setGlobalDrawerProps((props: any) => ({ visible: false })),
    }
    setGlobalDrawerProps(props);
}
...
const setRoleLimit = (params: ActionParamsModal) => {
    ...
    const props: UserDrawerProps = {
        ...
        title: <><span className={funcDefine.iconcls}></span> {msg}</>,
        width: '500px',
        children: <SetRoleLimits display={false}
            roleid={record[primarykey]} msg={msg} timestramp={new Date().getTime()} />,
        onClose: () => setGlobalDrawerProps((props: any) => ({ visible: false })),
    }
    setGlobalDrawerProps(props);
...
export const systemActions: ActionStore = apply({
    'createPersonnelUser': createPersonnelUser,
