...
const PmLiabilityYearColumn: React.FC = () => {
        ...
        request('/api/platform/datamining/fetchdata.do',
            {
                ...
                const dataSource = response.map(rec => ({
                    type: rec.text,
                    // 未支付
                    value: parseInt(numeral(rec.jf1cddb74923df9f536626517dddf / 10000).format('0')),
                    group: '当年度',
                })).sort((a, b) => a.type > b.type ? 1 : -1).
                    filter(rec => {
                        if (rec.type >= ('' + (parseInt(moment().format('YYYY')))))
                            return true;
                        else {
                ...
                if (thisyearbefore > 0) {
                    chartdata.splice(0, 0, {
                        type: parseInt(moment().format('YYYY')) + '年前',
    ...
    const config: ColumnConfig = {
        ...
        label: {
            ...
            formatter: (data) => {
                // 小于10万的不要显示值了，太小
                return data.value > 10 ? numeral(data.value).format('0,0') + '万' : '';
            },
        },
        tooltip: {
            formatter: (datum) => {
    ...
    return <Card title='根据付款计划测算的年度付款压力(隐性债务)柱状图' {...cardParams}>
        <Column {...config} />
    </Card>;
}
...
const PmAgreementPlanYearStackColumn: React.FC = () => {
        ...
        request('/api/platform/datamining/fetchdata.do',
            {
                ...
                const dataSource = response.map(rec => ({
                    type: rec.text,
                    value: parseInt(numeral(rec.jfae8913f5b24bdbb25b431582dab / 10000).format('0')),
                    payout: parseInt(numeral(rec.jf1208b6fed8ea0615d208003d004 / 10000).format('0')),
                    remain: parseInt(numeral(rec.jf1cddb74923df9f536626517dddf / 10000).format('0')),
                })).sort((a, b) => a.type > b.type ? 1 : -1).
                    filter(rec => rec.type > ('' + (parseInt(moment().format('YYYY')) - 7)));
                const chartdata: any[] = [];
                dataSource.forEach((rec) => {
                    chartdata.push({
                        type: rec.type,
    ...
    return <Card title='项目合同年度计划支付情况柱状图' {...cardParams}>
        <Column {...config} />
    </Card>;
};
...
const PmAgreementPlanYearColumn: React.FC = () => {
    ...
    const getStart = (): number => {
        return data.length > 13 ?
            Math.max((data.findIndex((rec, index) =>
                (rec.type as string) > moment().format('YYYY')) - 2) / data.length, 0)
            : 0
    };
    const getEnd = (): number => {
        return data.length > 13 ?
            (data.findIndex((rec, index) =>
                (rec.type as string) > moment().format('YYYY')) + 8.0) / data.length
            : 1
    }
    const config: ColumnConfig = {
        loading,
    ...
    return <Card title='项目合同年度度付款计划柱状图' {...cardParams}>
        <Column {...config} />
    </Card>;
};
...
const PmAgreementPlanYearMonthColumn: React.FC = () => {
    ...
    const getStart = (): number => {
        return data.length > 13 ?
            (data.findIndex((rec, index) =>
                (rec.type as string).startsWith(moment().format('YYYY'))) + 1.0) / data.length
            : 0
    };
    const getEnd = (): number => {
        return data.length > 13 ?
            (data.findIndex((rec, index) =>
                (rec.type as string).startsWith(moment().format('YYYY'))) + 12.0) / data.length
            : 1
    }
    const config: ColumnConfig = {
        loading,
    ...
    return <Card title='项目合同月度付款计划柱状图' {...cardParams}>
        <Column {...config} />
    </Card>;
};
