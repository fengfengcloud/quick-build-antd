    ...
    return <Row gutter={[12, 12]} style={{ margin: '12px 6px' }}>
        ...
        <Col span={24}>
            <Card title="合同文件审批表部门年度审批" {...cardParams}>
                <Org_Year_PmAgreementApproveColumn />
            </Card>
        </Col>
        <Col span={24}>
...
const OrganizationPmAgreementApprovePie: React.FC = () => {
        ...
        request('/api/platform/datamining/fetchdataminingdata.do',
            {
                ...
                setData(response.filter(rec => rec.moduleName === 'FOrganization').map(rec => ({
                    type: rec.text,
    ...
    const config: PieConfig = {
        ...
        label: {
            type: 'outer',
            formatter: (data) => {
                return numeral(data.value).format('0,0') + '个(' + numeral(data.percent).format('0.00%') + ')'
            },
        },
        statistic: {
            title: {
        ...
        tooltip: {
            formatter: (data) => {
                return { name: data.type, value: numeral(data.value).format('0,0') + '个\n' }
            },
        },
    };
    return <Card title={<React.Fragment>合同文件审批表部门审批
        <ToggleTableChartButton showGrid={showGrid} setShowGrid={setShowGrid} />
    </React.Fragment>} {...cardParams}>
        {showGrid ? <DataTable data={data} unitText="个"
            typeTitle="管理部门" valueTitle="文件数" /> :
            <Pie {...config} />}
    </Card>;
};
...
const YearPmAgreementApprovePie: React.FC = () => {
        ...
        request('/api/platform/datamining/fetchdataminingdata.do',
            {
                ...
                setData(response.filter(({ text }: { text: string }) =>
                    (text.length === 5 && text.endsWith("年")) || text === "空").map(rec => ({
    ...
    return <Card title={<React.Fragment>合同文件审批表年度审批
        <ToggleTableChartButton showGrid={showGrid} setShowGrid={setShowGrid} />
    </React.Fragment>} {...cardParams}>
        {showGrid ? <DataTable data={data} unitText="个"
            typeTitle="年度" valueTitle="文件数" /> :
            <Rose {...config} />}
    </Card>;
};

const Org_Year_PmAgreementApproveColumn: React.FC = () => {
    const [loading, setLoading] = useState<boolean>(true);
    const [data, setData] = useState<any[]>([]);
    useEffect(() => {
        setTimeout(() => {
        ...
        request('/api/platform/datamining/fetchdataminingdata.do',
            {
                ...
                setData(data.sort((a, b) => b.type > a.type ? -1 : a.type === b.type ? a.value - b.value : 1));
                setLoading(false);
            })
    };
