...
export const RowActionHistoryButton = () => {
    ...
    return (<><Button type={visible ? 'link' : 'text'} size="small"
        onClick={() => {
            setVisible(v => !v);
        }}><BlockOutlined /> 操作记录</Button>
        <Drawer
            title="数据分析方案的数据操作记录"
...
export const RowActionHistory = () => {
    ...
    const columns: any = [{
        ...
        width: 36,
        align: 'center',
        render: (value: any, record: any, recno: number) => <Tooltip title="从当前行开始删除所有后续操作"><DeleteOutlined
            onClick={() => {
    ...
    const { schemeState: { rowGroup, dataSource } } = state;
    ...
    let recno = 1;
    rowDataSource.forEach(row => row.recno = recno++)
    return <Table
