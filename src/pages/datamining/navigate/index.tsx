...
import { ACT_NAVIGATE_ADD_GROUP, ACT_NAVIGATE_ACTIVETAB_CHANGE, ACT_NAVIGATE_REMOVE_GROUP, ACT_TOGGLE_NAVIGATE_REGION } from '../constants';

export const DataminingNavigate = () => {
    const context = useContext<DataminingStateContext>(DataminingContext);
    const { state, dispatch } = context;
    const { navigates, currSetting: { navigate: { visible } } } = state;
    const navigateTrees = (
        <Tabs
    ...
    const menu = <Menu>{groupMenu(state.expandGroupFieldsTree || [], (group: any) => {
        ...
        <Resizable {...resizableProps}>
            <Layout.Sider theme='light' width='auto'>
                <Card
                    ...
                    size='small'
                >
                    {state.navigates.length == 0 ? <Empty style={{ marginTop: '48px' }} description="暂无导航分组" /> : null}
                    {navigateTrees}
                </Card>
            </Layout.Sider>
        </Resizable>
