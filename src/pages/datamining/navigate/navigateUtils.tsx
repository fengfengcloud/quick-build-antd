...
import {
    ACT_NAVIGATE_FETCH_DATA, ACT_NAVIGATE_ROW_EXPAND, CHILDREN, DRAG_ITEM_GROUPFIELD,
    MATCH_FIRST_POS, NAVIGATE_CHECK_CHANGE_DELAY, NAVIGATE_RECORD_COUNT_FIELD, NO_MATCH,
    ROWID, TEXT, TITLE, DRAG_NAVIGATE_RECORD
} from "../constants";
...
// 生成一个树形结构的所有选中记录的条件值
export const getNavigateTreeCondition = (navigate: DataminingNavigateModal): navigateConditionModal => {
    // 生成某一级的所有选中记录的条件
    // 从顶向下生成树
...
// 获取一个导航的所有条件值
export const getNaviagtesCondition = (navigates: DataminingNavigateModal[]): navigateConditionModal[] => {
...
export const NavigateRecordTitle = ({ navigateGroup, node, search }:
    { navigateGroup: ExpandGroupFieldModal, node: any, search?: string }) => {
        ...
        */
        drop: (item) => {
            expandNavigateRowWithGroup({
                state, dispatch, navigateGroup, expandRecord: node,
                groupFieldid: item.fieldid, groupTitle: item.title
            })
        },
        collect: (monitor) => {
            return {
    ...
    return <>
        ...
        <div ref={ref} className={isOver ? styles.dragover : ''} >
            {search ? <Highlighter highlightClassName="ant-btn-link"
                searchWords={[search]} textToHighlight={node.text} /> :
                (node.text || '未定义')}
 ...
 * @param nodes 
 */
export const genAllTreeData = (nodes: any[], navigateGroup: ExpandGroupFieldModal, parentNode: any,
    groupFieldid: string, groupTitle: string, expandedKeys: string[]): any => {
    const result: any[] = [];
    const getNode = (node: any, search: string = '') => {
        ...
        // 全自动生级的，加入当前级数
        if (node['level_'])
            result.groupfieldid = groupFieldid.replace('-all', '-' + node['level_']);
        else
            result.groupfieldid = groupFieldid;
        result.grouptitle = groupTitle;
        result.title = <NavigateRecordTitle navigateGroup={navigateGroup} node={result} search={search} />;
        return result;
    }

    result.push(...nodes.map((node: any) => {
        const { children } = node;
        ...
        if (Array.isArray(children) && children.length) {
            ...
            item.children = genAllTreeData(children, navigateGroup, item, groupFieldid, groupTitle, expandedKeys);
        }
        return item;
    }));
    return result;
...
const combineCondition = (conditions: navigateConditionModal[]): navigateConditionModal[] => {
    ...
    conditions.forEach(condition => {
        if (Array.isArray(condition.children)) {
            ...
            const firstPropertyChildren = result.find(cond => cond.property_ === condition.property_ && !cond.children);
...
export const fetchNavigateData = async (state: DataminingModal, navigate: DataminingNavigateModal,
...
// 按照某个分组展开导航数据时
export const expandNavigateRowWithGroup = ({ state, navigateGroup, expandRecord, groupFieldid,
    groupTitle, dispatch }: {
        state: DataminingModal, navigateGroup: ExpandGroupFieldModal,
        expandRecord: any, groupFieldid: string, groupTitle: string, dispatch: Function
    }) => {
    const { moduleName, filters } = state;
    // 从选中节点开始加入所有的父节点
    ...
    request('/api/platform/datamining/fetchdata.do', {
        ...
        const children = genAllTreeData(childrens, navigateGroup, expandRecord, groupFieldid, groupTitle, expandedKeys);
        if (expandRecord.children)
            (expandRecord.children as any[]).push(...children);
        else
            expandRecord.children = children;
        dispatch({
...
// 导航条件中选中和未选中的互换
export const toggleNavigateSelected = (state: DataminingModal, dispatch: Function,
            ...
            if (!hasChecked(record[ROWID])) {
                if (record.children) {
                    if (!childSomeChecked(record))
                        ...
                        // 有子节点，并且有一个选中的
                        arrangeChecked(record.children);
                } else
                    checked.push(record[ROWID])
            }
        })
    ...
    if (checkedKeys.length === 0) {
        checked.push('root');
    } else if (hasChecked('root')) { // 如果已经全选了，那么全清空

    } else {
        arrangeChecked(dataSource[0].children);
    }
    navigateCheckedChange(state, dispatch, navigate.navigateGroup.fieldid,
        checked, NAVIGATE_CHECK_CHANGE_DELAY);
}

// 导航条件中执行筛选操作
export const onNavigateSearch = (navigate: DataminingNavigateModal, search: string, dispatch: Function) => {
    const { navigateGroup } = navigate
    if (navigate.search == search)
        return;
    const searchChildren = (array: any[]) => {
        array.forEach(record => {
            const find = match(record.text, search);
            record[MATCH_FIRST_POS] = find ? find[0] : NO_MATCH;
            record[TITLE] = <NavigateRecordTitle node={record}
                navigateGroup={navigateGroup}
                search={find ? record[TEXT].substring(find[0], find[1] + 1) : ''} />;
            record.children && searchChildren(record.children);
        });
        ...
        const cancelSearchChildren = (array: any[]) => {
            array.forEach(record => {
                if (record[MATCH_FIRST_POS] !== NO_MATCH)
                    record[TITLE] = <NavigateRecordTitle node={record}
                        navigateGroup={navigateGroup} search={''} />;
                delete record[MATCH_FIRST_POS];
                record.children && cancelSearchChildren(record.children);
            });
        }
...
// 对导航树按照字段和顺序进行排序
export const navigateDataSourceSort = (navigate: DataminingNavigateModal, dispatch: Function,
    sortProperty: string, sortOrder: string) => {
    const { dataSource, navigateGroup } = navigate;
    const v = sortOrder === 'asc' ? 1 : -1;
    sortProperty = sortProperty === 'count' ? NAVIGATE_RECORD_COUNT_FIELD : sortProperty;
    const sortChildren = (array: any[]) => {
        array.sort((a, b) =>
            a[sortProperty] > b[sortProperty] ? v : a[sortProperty] < b[sortProperty] ? -v : 0)
...
export const getNavigateSelectedRecords = (state: DataminingModal, groupfieldid: string, sourceRecord: any): any[] => {
