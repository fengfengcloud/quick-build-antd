...
import { fetchNavigateData, genAllTreeData, navigateDataSourceSort, onNavigateSearch, toggleNavigateSelected } from './navigateUtils';
import {
    ACT_NAVIGATE_FETCH_DATA, ACT_NAVIGATE_EXPAND, ACT_NAVIGATE_CHECKED, ACT_NAVIGATE_AFTER_CHANGE,
...
export const NavigateTreeDiv: React.FC<any> = ({ navigate, state, dispatch }:
    { navigate: DataminingNavigateModal, state: DataminingModal, dispatch: Function }) => {
...
export const navigateCheckedChange = (state: DataminingModal,
    dispatch: Function, fieldid: string, checkedKeys: any[], delay: number) => {
    dispatch({
    ...
    if (navigate) {
        if ((checkedKeys.length === 0 || checkedKeys.find(key => key === 'root')) &&
...
export const NavigateTree: React.FC<any> = ({ navigate, state, dispatch }:
    { navigate: DataminingNavigateModal, state: DataminingModal, dispatch: Function }) => {
        ...
        if (navigate.dataSource.length === 0)
            fetchNavigateData(state, navigate, []).then((records: any[]) => {
                ...
                const dataSource = [{
                    ...
                    key: 'root',
                    rowid: 'root',
                    children: genAllTreeData(records, navigateGroup, null, navigateGroup.fieldid,
                        navigateGroup.title, expandedKeys),
                }]
                dispatch({
    ...
    const sortMenu = (
        <Menu onClick={({ key }) => {
            ...
            <Menu.Item key='count-desc' icon={EMPTY_MENU_ICON}>记录数由大到小</Menu.Item>
            <Menu.Item key='count-asc' icon={<SortAscendingOutlined />}>记录数由小到大</Menu.Item>
            <Menu.Item key='value-asc' icon={<SortAscendingOutlined />}>记录编码升序</Menu.Item>
            <Menu.Item key='value-desc' icon={EMPTY_MENU_ICON}>记录编码降序</Menu.Item>
            <Menu.Item key='text-asc' icon={<SortAscendingOutlined />}>记录文本升序</Menu.Item>
            <Menu.Item key='text-desc' icon={EMPTY_MENU_ICON}>记录文本降序</Menu.Item>
        </Menu>)
    return <>
        <div style={{ display: 'flex', marginBottom: '8px', marginTop: '-4px' }}>
            ...
            <Tooltip title="选中和未选中条件互换">
                <Button style={{ margin: '0px 6px' }} size='small' shape="circle"
                    onClick={() => {
                        toggleNavigateSelected(state, dispatch, navigate);
                    }}><RetweetOutlined />
                </Button>
            </Tooltip>
            <Search size='small' placeholder="搜索文本或简拼或全拼" style={{ flex: 1 }}
                defaultValue={navigate.search} allowClear
                onSearch={(search: string) => { onNavigateSearch(navigate, search, dispatch) }}>
            </Search>
        </div>
        ...
        <Tree
            ...
            onCheck={(checkedKeys: any) => {
                navigateCheckedChange(state, dispatch, navigateGroup.fieldid, checkedKeys,
                    NAVIGATE_CHECK_CHANGE_DELAY);
            }}
