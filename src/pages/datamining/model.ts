...
import {
    ACT_NAVIGATE_ADD_GROUP, ACT_NAVIGATE_FETCH_DATA, ACT_NAVIGATE_EXPAND,
    ACT_NAVIGATE_ACTIVETAB_CHANGE, ACT_NAVIGATE_SELECTED, ACT_NAVIGATE_REMOVE_GROUP,
    ROOTROWID, ACT_NAVIGATE_CHECKED, ACT_NAVIGATE_AFTER_CHANGE, ACT_NAVIGATE_ROW_EXPAND,
    ACT_FETCH_LOADING_CHANGED, ACT_MONETARY_CHANGED, ACT_TOGGLE_GROUP_REGION,
    ACT_TOGGLE_FILTER_REGION, ACT_TOGGLE_NAVIGATE_REGION, ACT_SELECTED_ROWKEYS_CHANGED,
    ACT_DATAMINING_EXPAND_CHANGED, ACT_CLEAR_ALL_ROWEXPAND, ACT_CLEAR_ALL_COLUMN_EXPAND,
    ACT_DELETE_ROWGROUP_FROM_INDEX, ACT_FILTER_DATASOURCE_UPDATE, ACT_UPDATE_DATAMINING_SCHEMEINFO,
    ACT_DATAMINING_FETCHDATA, ACT_DATAMINING_FETCH_SCHEMES, ACT_DATAMINING_CHANGE_SCHEME,
    ACT_FIELD_GROUP_UPDATE, ACT_COLUMN_GROUP_UPDATE, ACT_COLUMN_GROUP_REST_VISIBLE_TOGGLE,
    ACT_COLUMN_GROUP_EDIT_TEXT, ROWID, TEXT, ACT_COLUMN_GROUP_REMOVE, ACT_COLUMN_GROUP_VISIBLE_TOGGLE,
    ACT_COLUMN_GROUP_SELECTED_TOGGLE, SELECTED, ACT_COLUMN_GROPU_DESELECTEDALL,
    ACT_COLUMN_GROUP_COMBINE_SELECTED, ACT_FIELD_GROUP_REMOVE, ACT_FIELD_GROUP_HIDDEN_IN_TOGGLE,
    ACT_FIELD_GROUP_FIXED_LEFT_TOGGLE, ACT_FIELD_GROUP_ADD, ACT_SORT_CHANGE, ACT_ADD_SCHEME,
    ACT_DELETE_SCHEME, ACT_SETTING_CHANGE, ACT_DATAMINING_EXPORT_SETTING_CHANGE, ACT_SQLPARAM_CHANGE
} from "./constants";
...
import { clearColumnGroupAllSelected, combineSelectedColumns, removeRowidsFromTree } from "./resultTree/columnUtils";
    ...
    switch (type) {
        ...
        case ACT_DELETE_SCHEME:
            ...
            newState = update(state, {
                ...
                currentScheme: {
                    $set: newSchemes.length ? newSchemes[0] : {
                        schemeid: '',
                        text: '',
                        title: '',
                        savepath: true,
        ...
        case ACT_FIELD_GROUP_REMOVE:
            const fieldGroup: FieldModal[] = state.schemeState.fieldGroup.filter((field) => field[ROWID] != payload[ROWID]);
        ...
        case ACT_FIELD_GROUP_HIDDEN_IN_TOGGLE:
            const field = state.schemeState.fieldGroup.find((field) => field[ROWID] == payload[ROWID]);
        ...
        case ACT_COLUMN_GROUP_REMOVE:
            newState = update(state, {
                schemeState: {
                    columnGroup: {
                        $set: [...removeRowidsFromTree(state.schemeState.columnGroup, payload.rowids, payload.keepChildren)]
                    },
                }
            })
            break;
        ...
        case ACT_COLUMN_GROUP_COMBINE_SELECTED:
            const schemeState = combineSelectedColumns(state.schemeState.columnGroup, payload.keepChildren);
            if (schemeState)
                newState = update(state, {
                    schemeState: {
                        columnGroup: {
        ...
        case ACT_NAVIGATE_ADD_GROUP:
            if (!state.navigates.find(nav => nav.navigateGroup.fieldid === payload.navigateGroup.fieldid)) {
                newState = update(state, {
                    navigates: {
                        $push: [{
                            navigateGroup: payload.navigateGroup, dataSource: [],
                            expandedKeys: [], selectedKeys: [], checkedKeys: []
                        }]
                    }
                })
            }
        ...
        case ACT_NAVIGATE_FETCH_DATA:
            newState = update(state, {
                navigates: {
                    $apply: (navs: DataminingNavigateModal[]) => navs.map((nav: DataminingNavigateModal) => {
                        if (nav.navigateGroup.fieldid !== payload.fieldid)
                            return nav;
                        else
                            return {
                                ...nav,
        ...
        case ACT_NAVIGATE_EXPAND:
            newState = update(state, {
                navigates: {
                    $apply: (navs: DataminingNavigateModal[]) => navs.map((v) => {
                        if (v.navigateGroup.fieldid !== payload.fieldid)
                            return v;
                        else
            ...
            newState = update(state, {
                navigates: {
                    $set: state.navigates.map(record => {
                        ...
                        return record.navigateGroup.fieldid !== payload.navigateGroup.fieldid ? record :
                            {
                                ...record,
                                dataSource: [...record.dataSource],
                                expandedKeys: (expandedKeys.find(key => key === payload.key) ? expandedKeys :
                                    [...expandedKeys, payload.key]).concat(...payload.expandedKeys),
                            }
                    })
                }
            })
        ...
        case ACT_DATAMINING_FETCHDATA:
            ...
            if (payload.expandedRowKeys && payload.expandedRowKeys.length) {
                ...
                payload.expandedRowKeys.forEach((key: string) => {
                    if (!state.expandedRowKeys.find((e) => e === key))
                        keys.push(key)
                });
            ...
            if (payload.deletedRowids && payload.deletedRowids.length) {
                ...
                newState = update(newState, {
                    expandedRowKeys: {
                        $set: state.expandedRowKeys.filter((value) =>
                            !ids.find(id => id === value)
                        )
                    },
                    selectedRowKeys: {
                        $set: state.selectedRowKeys.filter((value) =>
                            !ids.find(id => id === value)
                        )
                    }
                })
        ...
        case ACT_DATAMINING_FETCH_SCHEMES:
            newState = fetchDataminingSchemes(state, action);
            break;
        ...
        case ACT_DELETE_ROWGROUP_FROM_INDEX:
            newState = update(newState, {
                schemeState: {
                    rowGroup: {
                        $apply: (group: any[]) => group.filter((_, index) =>
                            index < payload.deleteFormIndex)
                    }
                },
                refreshAllCount: {
                    $apply: c => c + 1
