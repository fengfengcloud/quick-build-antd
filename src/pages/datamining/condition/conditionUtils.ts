...
export const refreshFilterCount = (state: DataminingModal, dispatch: Function) => {
    ...
    filterDataSource.forEach(record => {
        const type = record['conditiontype'],
            originfilter = record['originfilter'];
        switch (type) {
            case 'viewscheme':
                condFilters.push({
                    type: 'viewscheme',
...
// 删除了一个条件
export const deleteFilterDataSourceRecord = (state: DataminingModal, dispatch: Function,
        ...
        dispatch({
            ...
            payload: {
                ...
                userfilter: userfilter.map(filter => {
                    if (filter.property === record.property)
...
export const getFilterDataSource = (filters: FiltersModal, filterDataSource: FilterDataSourceModal[]):
    ...
    if (viewscheme.viewschemeid) {
        ...
        if (index == -1)
            filterDataSource.splice(0, 0, filter);              //加进去
        else
            if (filterDataSource[index].originfilter.viewschemeid !== viewscheme.viewschemeid)
                filterDataSource.splice(index, 1, filter);      //替换掉
    } else {
        if (index != -1)
            filterDataSource.splice(index, 1);                  //删除
    }
    ...
    console.log(userFiltersWithTitle)

    filterDataSource = filterDataSource.filter(filter => {
        return filter.conditiontype !== 'userfilter' ||
    ...
    userFiltersWithTitle.forEach(ufilter => {
        ...
        const index = filterDataSource.findIndex(filter =>
            filter.conditiontype === 'userfilter' && filter.property === ufilter.property);
        if (index != -1)
            filterDataSource.splice(index, 1, filter);
        else
            filterDataSource.push(filter);
    })

    // 导航条件
    filterDataSource = filterDataSource.filter(filter => {
        return filter.conditiontype !== 'navigatefilter' ||
    ...
    navigatefilters.forEach(navfilter => {
        ...
        const index = filterDataSource.findIndex(filter =>
            filter.conditiontype === 'navigatefilter' && filter.property === navfilter.groupFieldid);
        if (index != -1)
            filterDataSource.splice(index, 1, filter);
        else
            filterDataSource.push(filter);
    ...
    filterDataSource.forEach(f => f.recordnum = -1);
