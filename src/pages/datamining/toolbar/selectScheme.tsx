...
import { AppstoreOutlined, CheckOutlined, DeleteOutlined, PlusOutlined, QuestionCircleOutlined, SaveOutlined } from '@ant-design/icons';
...
const SelectScheme: React.FC = () => {
    ...
    const actionMenu = [
        <Menu.Item key='addscheme' icon={<PlusOutlined />}
            onClick={() => {
                saveNewScheme(state, dispatch);
            }}
        >当前数据分析方案另存为</Menu.Item>,
        <Menu.Item key='savescheme' icon={<SaveOutlined />} disabled={state.schemes.length === 0}
            onClick={() => {
                saveEditScheme(state, dispatch);
            }}
        >保存到当前数据分析方案</Menu.Item>,
        <Menu.Item key='deletescheme' icon={<DeleteOutlined />} disabled={state.schemes.length === 0}
            onClick={() =>
                Modal.confirm({
                    title: '确定删除',
        ...
        >删除当前数据分析方案</Menu.Item>,
        <Menu.ItemGroup key='schemes' title='所有数据分析方案'>
            {state.schemes.length == 0 ?
                <Menu.Item key='_noscheme_' disabled>暂无数据分析方案</Menu.Item> :
                state.schemes.map(scheme =>
                    <Menu.Item key={scheme.schemeid}
                        icon={currentScheme?.schemeid === scheme.schemeid ?
                            <CheckOutlined /> :
    ...
    return <Dropdown overlay={menu}>
        <Button type='text' size='small'><AppstoreOutlined />分析方案</Button>
    </Dropdown>
