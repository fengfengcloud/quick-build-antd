...
import {
    ACT_COLUMN_GROUP_EDIT_TEXT, ACT_COLUMN_GROPU_DESELECTEDALL, PARENT_ROWID, ROWID, ACT_COLUMN_GROUP_REMOVE,
    ACT_COLUMN_GROUP_VISIBLE_TOGGLE, ACT_COLUMN_GROUP_SELECTED_TOGGLE, ACT_COLUMN_GROUP_COMBINE_SELECTED,
    ACT_COLUMN_GROUP_REST_VISIBLE_TOGGLE, SELECTED, TEXT, ACT_FIELD_GROUP_REMOVE, ACT_FIELD_GROUP_HIDDEN_IN_TOGGLE,
    DRAG_ITEM_GROUPFIELD, ACT_FIELD_GROUP_FIXED_LEFT_TOGGLE, ACT_FIELD_GROUP_ADD, DRAG_NAVIGATE_RECORD,
} from '../constants';
import { getTreeSelectedKeys } from '../utils';
import {
    ApartmentOutlined, DeleteOutlined, DeleteRowOutlined, EyeInvisibleOutlined, EyeOutlined, MergeCellsOutlined,
    NodeExpandOutlined,
    PlusOutlined,
    ToTopOutlined
} from '@ant-design/icons';
import { EMPTY_MENU_ICON } from '@/utils/utils';
import { expandHeadCellWithGroup, getCellType, moveColumnGroup, moveFieldGroup, expandHeadCellWithNavigate } from './columnUtils';
 ...
 * @param param0 
 */
export const DragDropHeaderCell = ({ children, column, className, ...restProps }:
    { column: any, children: ReactChild, className: string, restProps: any }) => {
    ...
    const canMoveDrag: boolean =
        ...
        (cellType === 'group' || cellType === 'fieldingroup');
    ...
    const [{ isMoveOver, canMoveDrop, dropClassName }, moveDrop] = useDrop({
        ...
        canDrop: (item, monitor) => {
            ...
            const { isTotalColumn: isDragTotalColumn, [ROWID]: dragRowid, [PARENT_ROWID]: dragParentRowid }
                = dragColumn || {};
            // 拖动和目标不相同
            return (dragRowid !== column[ROWID]) &&
                (
        ...
        collect: monitor => {
            const { column: dragColumn } = monitor.getItem() || {};
            const { rowid: dragRowid, isTotalColumn: isDragTotalColumn, [PARENT_ROWID]: dragParentRowid } = dragColumn || {};
            ...
            return {
                ...
                // 在相同的父节点下才可以进行记录的拖动
                canMoveDrop: (dragRowid !== column[ROWID]) &&
                    (
                        (isDragTotalColumn && column.isTotalColumn) ||
                        dragParentRowid === column[PARENT_ROWID]
                    ),
    ...
    const [{ isNavigateOver, }, navigateDrop] = useDrop({
        ...
        drop: (item) => {
            ...
            expandHeadCellWithNavigate(state, dispatch, cellType,
                (cellType === 'group' || cellType === 'fieldingroup') ? column : {},
                item.navigateGroup.fieldid, item.navigateGroup.text || item.navigateGroup.title, sourceRecords);
        },
        collect: (monitor) => {
            return {
    ...
    // 判断哪些是可以展开的
    if (cellType === 'sumfield' || cellType === 'fieldingroup' || cellType === 'group' && column.condition)
    ...
    // 所有的合并分组的右键菜单
    const columnGroupMenu = <Menu>
        {/* 单个聚合字段的末级，无法双击，只能用菜单来选择 */}
        {cellType === 'fieldingroup' ?
            <Menu.Item key='toggleselected' icon={EMPTY_MENU_ICON}
                onClick={toggleCellSelected} >{column[SELECTED] ? '取消选中当前分组' : '选中当前分组'}
            </Menu.Item> : null
        }
        {selectCount >= 2 || selectCount === 1 && !column[SELECTED] ?
            <Menu.Item key='clearallselected' icon={EMPTY_MENU_ICON}
                onClick={() => {
                    dispatch({
        ...
        <Menu.Item key='togglevisible' icon={<EyeInvisibleOutlined />}
            onClick={() => {
                dispatch({
                    ...
                    payload: {
                        type: 'hidden',
                        rowids: selectCount ?
                            getTreeSelectedKeys(state.schemeState.columnGroup, ROWID, SELECTED) : [column[ROWID]],
                    }
                })
            }} > {(selectCount ? `隐藏选中的 ${selectCount} 个分组` : `隐藏当前分组`)}
        </Menu.Item>
        <Menu.Item key='hiddenrest' icon={<ToTopOutlined rotate={column.hideRest ? 90 : 270} />}
            onClick={() => {
                dispatch({
                    // 当前节点后面的同级节点都不显示
        ...
        {cellType === 'fieldingroup' || cellType === 'group' && column.condition ?
            <>
                <Menu.SubMenu key="fieldexpand"
                    icon={<NodeExpandOutlined />}
                    title={'按分组展开'}>
                    {groupMenu(state.expandGroupFieldsTree || [], (group: any) => {
                        expandHeadCellWithGroup(state, dispatch, cellType, column, group.fieldid, group.text || group.title)
                    })}
                </Menu.SubMenu>
                <Menu.Divider />
            </>
        ...
        <Menu.Item key='combine' icon={<MergeCellsOutlined />}
            disabled={selectCount <= 1}
            onClick={() => combineSelected(false)} >{`合并选中的 ${selectCount} 个分组`}
        </Menu.Item>
        <Menu.Item key='combineadd' icon={<ApartmentOutlined />}
            disabled={selectCount <= 1}
            onClick={() => combineSelected(true)} >{`合并选中的 ${selectCount} 个后加入原分组`}
        </Menu.Item>
        {/* 修改分组描述 */}
        <Menu.Item key='edittext' icon={EMPTY_MENU_ICON}
            onClick={() => {
                ...
                Modal.confirm({
                    ...
                    content: <Form autoComplete='off'>
                        <Input name="text" defaultValue={column.text} maxLength={100}
        ...
        <Menu.Divider />
        {/* 删除当前列或选中列（保留子分组） */}
        <Menu.Item key='removecolumnkeepchildren' icon={<DeleteRowOutlined />}
            onClick={() => { removeColumnGroup(true) }}>
            {(selectCount ? `仅删除选中的 ${selectCount} 个分组` : `仅删除当前分组`) + '(保留子分组)'}
        </Menu.Item>
        {/* 删除当前列或选中列，包括所有子分组 */}
        <Menu.Item key='removecolumn' icon={<DeleteOutlined />}
            onClick={() => { removeColumnGroup(false) }}>
    ...
    const fieldGroupMenu = <Menu>
        <Menu.SubMenu key="fieldexpand"
            icon={<NodeExpandOutlined />}
            title={'聚合字段按分组展开'}>
            {groupMenu(state.expandGroupFieldsTree || [], (group: any) => {
                expandHeadCellWithGroup(state, dispatch, cellType, {}, group.fieldid, group.text || group.title)
            })}
        </Menu.SubMenu>
        <Menu.Divider />
        <Menu.Item key={'fieldGroupFixedLeft'} icon={EMPTY_MENU_ICON}
            onClick={() => {
                dispatch({
        ...
        <Menu.Divider />
        <Menu.Item key={ACT_FIELD_GROUP_HIDDEN_IN_TOGGLE}
        ...
        <Menu.SubMenu key="addagreegatefield"
            icon={<PlusOutlined />}
            title={'增加聚合字段'}>
        ...
        <Menu.Item key={ACT_FIELD_GROUP_REMOVE} icon={<DeleteOutlined />}
            disabled={state.schemeState.fieldGroup.length <= 1}
            onClick={() => {
                dispatch({
    ...
    const th = <th ref={ref} onDoubleClick={toggleCellSelected}
        children={children}
        {...restProps}
        className={className + (isMoveOver && canMoveDrop ? ' ' + dropClassName : '') +
            (isGroupExpandFieldOver || isNavigateOver ? ' groupexpandoverheadercell' : '')}
    />
    return column.hidden || cellType === 'subfield' ? th :
        <Dropdown overlay={cellType === 'sumfield' ? fieldGroupMenu : columnGroupMenu} trigger={['contextMenu']}>
            {th}
        </Dropdown>
