...
export const selections = (state: DataminingModal, dispatch: Function) => {
    ...
    return [
        {
            ...
            text: <><CheckSquareOutlined />全选所有记录</>,
            onSelect: () => {
                updateSelections(getAllChildRowids({ children: dataSource }))
            },
        }, {
            key: 'selected_clear',
            text: <><BorderOutlined />取消所有选中记录</>,
            onSelect: () => { updateSelections([]) }
        }, {
            key: Table.SELECTION_INVERT,
            text: <><SwapOutlined />反选所有记录</>,
            onSelect: () => {
                updateSelections(
                    getAllChildRowids({ children: dataSource }).filter(key =>
                        !state.selectedRowKeys.find(sk => sk === key))
                )
            }
        }, {
            key: 'select_allleaf',
