...
import {
  combineSelectedRows, deleteSelectedRows, deleteSelectedRowsChildren, expandRowsWithGroup,
} from '../rowActionUtils';
import groupMenu from '../toolbar/groupMenu';
import {
  ClearOutlined, DeleteRowOutlined, NodeCollapseOutlined, NodeExpandOutlined, ScissorOutlined,
  SisternodeOutlined, SubnodeOutlined, UndoOutlined
} from '@ant-design/icons';
import { EMPTY_MENU_ICON } from '@/utils/utils';
import {
  ACT_CLEAR_ALL_ROWEXPAND, ACT_DATAMINING_FETCHDATA, ACT_SELECTED_ROWKEYS_CHANGED, CHILDREN,
  ACT_DELETE_ROWGROUP_FROM_INDEX, ROWID, TITLE, ROOTROWID
} from '../constants';
 ...
 * @param param3
 */
export const categoryFieldRender = (value: any, record: Object, recno: number) => {
  return <CategoryField value={value} record={record} recno={recno} key={record[ROWID]} />;
};

const CategoryField = ({ value, record, recno }: { value: any; record: Object; recno: number }) => {
  const context = useContext<DataminingStateContext>(DataminingContext);
  const { state, dispatch } = context;
  const selectCount = state.selectedRowKeys.length;
  const { schemeState: { rowGroup } } = state;
  // 右键弹出在category上的context菜单
  const menu = <Menu onClick={({ domEvent }) => {
    domEvent.stopPropagation();
  }}>
    {/* 如果当前记录是选中记录，则展开所有选中记录，否则只展开当前记录 */}
    <Menu.SubMenu key="expandselected"
    ...
    <Menu.Item key='combine' icon={<NodeCollapseOutlined />}
      disabled={selectCount <= 1}
      onClick={() => {
        combineSelectedRows(state, dispatch, false);
      }} >合并选中行</Menu.Item>
    <Menu.Item key='combineadd' icon={<SisternodeOutlined rotate={180} />}
      disabled={selectCount <= 1}
      onClick={() => {
    ...
    <Menu.Item key='edittext' icon={EMPTY_MENU_ICON}
      onClick={() => {
        ...
        Modal.confirm({
          title: '请输入分组描述',
          icon: null,
          content: <Form autoComplete='off'><Input name="text" defaultValue={value} maxLength={100}
    ...
    <Menu.Divider />
    <Menu.Item key='clearrow' icon={<DeleteRowOutlined />}
      disabled={!state.schemeState.dataSource[0].children}
      onClick={() => deleteSelectedRows({ state, dispatch, deletedRecord: selectCount ? null : record })}>
      {selectCount ? `删除选中行` : `删除当前行`}
    </Menu.Item>
    <Menu.Item key='clearrowchildren' icon={<ScissorOutlined />}
      disabled={selectCount === 0 && !record[CHILDREN]}
      onClick={() => deleteSelectedRowsChildren({ state, dispatch, deletedRecord: selectCount ? null : record })}
    >{selectCount ? `删除选中行` : `删除当前行`}的子节点</Menu.Item>
    <Menu.Divider />
    {selectCount ?
    ...
    <Menu.Item key='undo' icon={<UndoOutlined />} disabled={rowGroup.length === 0}
      onClick={() => {
        const last = rowGroup[rowGroup.length - 1];
        const type = rowOperTypes[last.type];
        message.info('最后一次行操作:' + type + ' ' + (last[TITLE] || '') + ' ' +
          (last.conditiontext || last.text || '') + ' 已撤消！');
        dispatch({
    ...
    <Menu.Item key='clearallrow' icon={<ClearOutlined />}
      disabled={!state.schemeState.dataSource[0].children}
      onClick={() => {
        dispatch({
  ...
  return (
    <>
      <Dropdown overlay={menu} trigger={['contextMenu']}>
        <div className={columnStyles.manytoonefield}
          style={{ flex: 1, transition: 'all 1s', padding: '8px 0px', margin: '-8px 0px' }}>
          <span className={columnStyles.manytoonefieldvalue} 
