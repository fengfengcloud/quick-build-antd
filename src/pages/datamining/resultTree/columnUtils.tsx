...
import {
  ACT_COLUMN_GROUP_VISIBLE_TOGGLE, ACT_COLUMN_GROUP_REST_VISIBLE_TOGGLE, ROWID, SELECTED, TEXT,
  ACT_FIELD_GROUP_UPDATE, ACT_COLUMN_GROUP_UPDATE, PARENT_ROWID
} from '../constants';
...
export const getCellType = (column: any): HeaderCellType =>
  (column[ROWID] as string).startsWith('field') ? 'sumfield' :
    (column[ROWID] as string).indexOf('field') > 0 ? 'subfield' :
      column['dataIndex'] ? 'fieldingroup' : 'group';
...
export const adjustColumnGroupToggleButton = (columns: any[], dispatch: Function) => {
  for (let i = columns.length - 1; i >= 0; i--) {
    ...
    if (column.hidden) {
      column.title = <><Tooltip title={'显示 ' + column.text}>
        <div style={cellType === 'fieldingroup' ? { marginLeft: '-5px', marginRight: '-5px' } :
        ...
        </div></Tooltip>{cellType === 'fieldingroup' ? null : <>&nbsp;</>}</>;
      column.width = '21px';
    ...
    } else if (column.hideRest || (i != columns.length - 1 && !!column.children && column.text == '小计')) {
      // 如果当前记录是小计，并且后面还有column，说明这是一级展的记录，后面是另一个分组的展开，可以显示或隐藏
      // 不是小计的记录可以通过菜单里面实现这个功能，这里就要判断一下了
      //delete column.children;
      column.title = <span style={{ display: 'flex' }}>
        <span style={{ flex: 1 }}>{column.text}</span>
        <Button type='text' size='small' style={{ padding: '0px' }}
          onClick={() => {
            dispatch({
              // 当前节点后面的同级节点都不显示
          ...
          {column.hideRest ?
            <><ToTopOutlined rotate={90} />
              <Tooltip title={getRestTitles(columns, i + 1)}>
                <Badge count={columns.length - i - 1} size='small' offset={[-5, -2]}
                  style={{ backgroundColor: '#69c0ff', boxShadow: '0 0 0 1px #1890ff' }} />
              </Tooltip>
            </> :
            <Tooltip title='隐藏后面剩余的分组'><ToTopOutlined rotate={270} /></Tooltip>}
        </Button>
      </span>;
      if (column.hideRest)
        columns.splice(i + 1, columns.length - (i + 1));
    } else if (cellType === 'group') {
      column.title = <>{column.text}
        <span className={styles.celltypegroup}>
          <DoubleLeftOutlined className={styles.eye} onClick={() => {
            dispatch({
              type: ACT_COLUMN_GROUP_VISIBLE_TOGGLE,
              payload: {
                type: 'hidden',
    ...
    if (column.children)
      adjustColumnGroupToggleButton(column.children, dispatch);
  }
}
...
const removeRowidFromTree = (records: any[], rowid: string, keepChildren: boolean) => {
  let removed = false;
  const doRemove = (records: any[]) => {
    if (removed) return;
    for (let i = 0; i < records.length; i++) {
      const rec = records[i];
      if (rec[ROWID] === rowid) {
        if (rec.children && keepChildren)
          records.splice(i, 1, ...rec.children);
        else
          records.splice(i, 1);
        removed = true;
        return;
...
export const removeRowidsFromTree = (records: any[], rowids: string[], keepChildren: boolean) => {
  rowids.forEach(rowid => removeRowidFromTree(records, rowid, keepChildren));
  return records;
}

// 清除所有选中的
export const clearColumnGroupAllSelected = (records: any[]): any[] => {
  const clearSelected = (records: any[]) => {
    records.forEach(rec => {
      delete rec[SELECTED];
      rec.children && clearSelected(rec.children);
    });
  }
...
export const getSelectedColumnGroup = (records: any[]): any[] => {
  const result: any[] = [];
  const getSelected = (records: any[]) => {
    records.forEach(rec => {
      if (rec[SELECTED]) result.push(rec);
      rec.children && getSelected(rec.children);
    })
  };
  getSelected(records);
  return result;
 ...
 */
export const combineSelectedColumns = (columnGroup: any[], keepChildren: boolean): any[] | false => {
  ...
  let text: string = '',
    ...
    first = selections[0],
    firstcondition = selections[0]['condition'],
    ahead = firstcondition.substring(0, firstcondition.lastIndexOf('=')),
  ...
  for (let i in selections) {
    ...
    // field=value
    const condition = selections[i]['condition'],
  ...
  parentChildren.splice(parentChildren.findIndex(rec => rec == first), 0, rec);
  selections.forEach(selection => parentChildren.splice(parentChildren.findIndex(r => r == selection), 1));
  return columnGroup;
}

// 聚合字段进行拖动了之后进行排序，聚合字段只能在总计列进行拖动
export const moveFieldGroup = (state: DataminingModal, dispatch: Function, dragRowid: string, dropRowid: string) => {
  let fieldGroup: any[] = [...state.schemeState.fieldGroup];
...
export const moveColumnGroup = (state: DataminingModal, dispatch: Function, parentid: string,
  dragRowid: string, dropRowid: string) => {
  const { schemeState: { columnGroup } } = state;
...
const adjustChildrenNodes = (children: any[], fieldid: string, pid: string): any[] => {
  ...
  children.forEach(child => {
    const condition = pid + fieldid + (child.level_ ? '-' + child.level_ : '') + '=' + child.value;
    const column: any = {
      // condition分组条件
...
export const expandHeadCellWithGroup = (state: DataminingModal, dispatch: Function, cellType: HeaderCellType,
  expandNode: any, fieldid: string, title: string) => {
  getFieldColumnWithGroup(state, fieldid, title, expandNode).
    then(columns => {
      if (columns) {
...
export const expandHeadCellWithNavigate = (state: DataminingModal, dispatch: Function, cellType: HeaderCellType,
  expandNode: any, fieldid: string, title: string, navigateReocrds: any[]) => {
  ...
  if (cellType === 'sumfield') {
    ...
    if (fieldid !== targetLastFieldid) {
      const columnhj = { ...column, text: '小计' };
      delete column.condition;  // 删除条件，最顶层的不可以再展开了，要在小计上展开
      column.children = [columnhj, {
        text: title, leaf: false,
        children: records.map(rec => ({
          ...rec, condition: targetCondition + '|||' + rec.condition
        }))
      }];
    } else {
      // 加到当前分组的后面
      ...
      parentChildren.splice(parentChildren.findIndex((node: any) => node[ROWID] == expandNode[ROWID]) + 1, 0,
        ...records.map((rec: any) => ({
          ...rec,
 ...
 * @param parentnode 
 */
export const getFieldColumnWithGroup = async (state: DataminingModal, fieldid: string, title: string, expandNode: any) => {
  ...
  if (fieldid.endsWith('-auto')) {
    ...
    const nextlevel = findParentLastLevel(fieldid_, pid ? pid.split('|||') : []) + 1;
  ...
  if (maxcol > 1 && columns.length > maxcol) {
    ...
    if (columns.length - maxcol < 3) {
      // 3个以内
      for (let i = maxcol; i < columns.length; i++)
  ...
  else if (setting.expandColAddGroupName === 'yes') {
    return [{
      text: title,
      children: addcolumns,
      leaf: false
