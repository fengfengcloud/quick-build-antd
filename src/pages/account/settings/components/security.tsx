...
const SecurityView = ({ user, dispatch }: { user: any, dispatch: Dispatch }) => {
    ...
    const getStrong = (value: string) => {
        ...
        setValidateStatus(level == 0 ? 'error' : level == 1 ? 'warning' : level == 2 ? 'success' : 'success');
        setHelp(level == 0 ? '密码最少6个字符' : level == 1 ? '密码强度弱' : level == 2 ? '密码强度中' : '密码强度强')
    }
    ...
    return <Card title="安全设置" bordered={false} >
        <List
        ...
        />
        <Modal title={<span className="x-fa fa-user-secret"> 修改密码</span>}
            destroyOnClose
            visible={visible}
        ...
        >
            <Card bordered={false} bodyStyle={{ padding: 0, margin: 0 }}>
                <Form form={form} labelCol={{ span: 8 }} wrapperCol={{ span: 12 }} >
                    ...
                    <Form.Item label="确认密码" name="newPasswordagain" hasFeedback
