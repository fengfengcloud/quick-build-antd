...
const BaseView = ({ personnel, dispatch }: { personnel: any, dispatch: Dispatch }) => {
    ...
    const onProvinceSelected = (value: string) => {
        ...
        if (citys)
            setCityOptions(city[value as string].
    ...
    useEffect(() => {
        if (personnel) {
            ...
            if (personnel.province)
                onProvinceSelected(personnel.province);
        ...
        <Card title="基本设置" bordered={false} >
            <Form layout="vertical" form={form} >
                <Row gutter={96}>
                    <Col flex="500px">
                        <Form.Item
                            label={<>手机号码<Text style={{ marginLeft: '24px' }}
                                type='secondary'>(非常重要，用于手机登录和接受短消息)</Text></>}
                            name="mobile" >
                            <Input />
                        </Form.Item>
                        ...
                        <Form.Item noStyle>
                            <Form.Item name="province" label="所在省市"
                                ...
                                <Select showSearch optionFilterProp="label"
                                    onSelect={value => onProvinceSelected(value as string)}
                ...
                <Form.Item>
                    <Button type="primary" loading={saving} onClick={onSavingSetting}>更新基本信息</Button>
                </Form.Item>
            </Form>
        </Card>
    )
